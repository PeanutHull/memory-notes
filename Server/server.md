   ...
   - 运维
     ...
     1. 模板配置
        ...
        - `templates/xx/xx/config.tmpl`：模板内容
            ...
            ```
1. etcd
   - 认识：分布式一致性解决方案，是一个高可用的分布式键值数据库，可用于分布式系统，从web程序到kubernetes、openstack都在用，基于go，采用raft协议作为一致性算法
...
1. 比较
   ...
   - Etcd：一个开源的、分布式的键值对数据存储系统，提供共享配置、服务的注册和发现。etcd内部采用raft协议作为一致性算法，etcd基于Go语言实现，etcd与zookeeper相比算是轻量级系统，两者的一致性协议也一样，etcd的raft比zookeeper的paxos简单。安装配置简单，有丰富的http api,支持SSL证书验证，ETCD 被广泛用在分布式系统中，如K8S中，还有各种微服务中。
