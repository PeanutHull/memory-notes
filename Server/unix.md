...
1. 多进程编程模型
   - 唯一id标识符：可重用，存在延迟复用算法保证无误认
     ...
     1. id为2：页守护进程
   - 进程状态：就绪(具备条件、cpu没来)、运行(内核态、用户态)、阻塞
...
1. 认识：是软件中断，是处理异步事件的方法，逻辑是当XX发生时，执行以下操作。POSIX.1有了标准化的可靠信号机制
   ...
   - 对信号的处理方式
     1. 忽略
     1. 捕捉：通知内核X信号时调用用户函数
     1. 执行系统默认动作：有记录内存映像core文件、忽略，大多数默认终止进程
1. 特性
...
1. 函数
   ...
   - kill：指定pid、同一进程组、进程组id、所有权限进程，权限为除超级用户外，用户id必须相等
   - raise：允许向自身发送信号
   - alarm：一个进程只能有一个闹钟时间多次调用替换，由于进程调度，时间会多，秒数
   - pause：使进程挂起，直到捕捉到信号
   - sigqueue：只能把信号发给单进程，可传递额外数据，有数量限制
   - about、system：采用信号实现
   - 睡眠函数：sleep、nanosleep、clock_nanosleep：避免了竞争条件(使用longjmp)，但是未处理与之前闹钟的交互作用，纳秒级精度，基于特定时钟的延迟时间，相对休眠更不准
   - 作业控制信号：SIGCHLD、SIGCONT、SIGSTOP、SIGTSTP、SIGTTIN、SIGTTOU
...
1. wiki
   - 信号举例
     1. SIGKILL：和SIGSTOP不可忽略，因为提供了进程终止的可靠方法，也因为忽略硬件异常信号的话进程的运行行为未定义，也不可捕捉
     1. SIGTERM：系统默认终止信号，kill产生

     1. SIGINT：中断信号，用户按中断键触发
     1. SIGQUIT：退出信号，同时产生core文件
