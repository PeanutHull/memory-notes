### 认识
1. 认识：基于事件驱动、高性能非阻塞的轻量级http/反向代理/负载均衡服务器。c编写
   - 高并发，高性能
     1. 32核64G，千万qps，简单静态文件百万qps
   - 高可靠性，运行稳定，热部署
   - 资源消耗低
...
1. 指令
   ...
   - 语法
     1. #注释，$变量，;结尾
     1. 指令与参数空格分隔，部分指令参数支持正则
     1. 结构：main-->http-->server-->location
     1. 指令块用{}组织，可以有名称
     1. include：组合另外的配置文件
   - 组成：每个指令的组成有三个方面
     1. syntax：语法形式
   ...
   - 操作
     1. 指令的合并
        ...
        - 动作指令：不可以
   - 配置文件
     1. mime.types：文件扩展名与文件类型映射表，找不到使用默认default_type
     1. fastcgi.conf/fastcgi_params/uwsgi_params/scgi_params：使用对应cgi时，向cgi传递的变量
     1. koi-utf/koi-win/win-utf：编码转换映射文件
   - nginx.conf
    ```conf
    user                        www;                            # 用户和用户组
...
1. 连接控制
   - 超时时间
     1. proxy_connect_timeout：和后端服务器的连接(发起握手后的)等待超时时间
     1. proxy_read_timeout：等待后端服务器的响应超时时间
     1. proxy_send_timeout：发送请求给后端服务器的超时时间，规定时间之内后端服务器接收完所有的数据
   - keepalive控制
     1. keepalive_disable：指定某些浏览器禁用
     1. keepalive_requests：一个tcp最多执行的http请求
     1. keepalive_timeout timeout [header_timeout]：两个，没有http请求的超时时间和连接的保留时间
   - 使用长连接：降低和上游建立/关闭连接的损耗，提升吞吐量，降低时延
    ```lua
    server {
    ...
    ```
   - 缓存控制
     1. proxy_buffer_size：缓存区大小
     1. proxy_buffers：缓存区大小和数量
     1. proxy_busy_buffers_size：高负荷下缓存大小
     1. proxy_temp_file_write_size：缓存临时文件大小

     1. client_max_body_size 500m;              # 客户端请求服务器最大允许大小
     1. client_body_buffer_size     128k;       # nginx分配给请求数据的Buffer大小
     1. proxy_ignore_client_abort   on;         # 是否开启proxy忽略客户端中断
1. 防盗链
...
1. 接口网关
   - 必备功能
     1. http、https、websocket反向代理
     1. 用户鉴权：匿名、Basic、Apikey、JWT、AK/SK认证
     1. 日志：流量日志，运行日志
     1. 数据统计：可导出到promethus、Graphite中进行分析
     1. 插件
        - 限流
     1. 高性能
   - 必备特性
     ...
     1. 监控报警
   - 升级功能
     1. 动态路由：通过设置location、query、header、host、method等参数匹配对应服务
     1. 针对gRPC的协议转换
     1. http请求数据重写
   - 升级特性
     1. 网关本身特性：可看nginx分类下的wiki
        - 负载均衡
        - 健康检查
     1. 服务发现：对接Eureka、Nacos、Consul
     1. 服务编排：一个编排API对应多个backend，backend的入参支持客户端传入，也支持backend间的参数传递；backend的返回数据支持字段的过滤、删除、移动、重命名、拆包和封包；编排API能够设定编排调用失败时的异常返回
     1. 配套后台
   - 其他
   - 现状
     1. tyk：老牌，性能好像不行
     1. Janus：轻量
     1. Apinto/Goku：国产开源，有商业版本
...
1. 马蜂窝广告数据处理
   - 收集
     1. lua代码缓存，设置Resolver、epoll、keepalive
     1. 数据收集部分不需要考虑时序或对数据进行聚合处理，因此核心的推送介质选择 Lua 共享内存即可，以 I/O 请求来代替访问其他中间件所需要的网络服务
   - 处理
     1. FFI处理ip
     1. 之后创建内部的日志 location，结合 Lua 自定义 log_format，利用 Nginx 子请求特性离线完成数据落盘，同时保证数据延迟时长在毫秒级
     1. Redis+FluxDB
        - 去中心化，配置链接池来增加链接复用，增加 AOF与延时入库保证可靠
        - FluxDB保证数据日志时序性可查，聚合统计与实时报表表现较优
   - 存储
     1. 实时数据源：数据采集服务→ Filebeat → Kafka → Flink → ES
     1. 离线数据源：HDFS → Spark → Hive → ES
   - 流程
     1. init_worker_by_lua阶段：负责服务配置业务
     1. access_by_lua阶段：负责CC防护、权限准入、流量时序监控等业务
     1. content_by_lua阶段：负责实现限速器、分流器、WebAPI、流量采集等业务
     1. log_by_lua阶段：负责日志落盘等业务
   - 分流器业务：NodeJS上报cpu、内存使用情况；Lua脚本调用RedisCluster获取时间窗口内NodeJS集群使用情况，计算出负载较高的，进行熔断、降级、限流等处理；将监控数据同步InfluxDB，进行时序监测
   - 小型web防火墙：使用第三方开源 lua_resty_waf 类库实现，支持 IP 白名单和黑名单、URL 白名单、UA 过滤、CC 攻击防护功能。在此基础上增加WAF对InfluxDB 的支持，进行时序监控和服务预警
1. 有赞网关
   - 架构：Jetty部署
...
### 原理
1. 设计
   - 进程模型：管理进程(master)和工作进程(worker)
   - 模块化设计：主框架只提供核心代码，各模块继承同套标准接口规范和数据结构，模块进行分层，为核心/配置/事件/HTTP/mail
   - 技术点：内存池，连接池，自旋锁，红黑树、事件驱动
1. 进程模型
   - 认识
     1. 管理进程和工作进程分开
     1. 是多进程结构，多线程，会共享地址空间，如果段错误越界访问进程都挂掉，不安全
     1. 进程间通信使用共享内存，如缓存模块
   ...
   - 进程结构
     ...
     1. 组成
        ...
        - worker
1. 事件驱动
   - 认识：event模块，采用红黑树管理事件定时器，支持各种事件驱动模型
     1. 非阻塞事件驱动处理器，线程池处理磁盘阻塞调用
     1. epoll：大量连接中只有少数是活跃的，epoll仅遍历活跃连接，采用红黑树、链表，增删改查都快
...
1. 网络模型
   - 认识
     1. apache是线程池实现多请求，占用资源多；nginx是io多路复用
...
1. 其他
   ...
   - 依赖组件
     ...
     1. openssl
   - 代理
     1. 正向代理：面向client，server可以感知不到
     1. 反向代理：面向server，client始终和反向代理通信，client可以感知不到
   - windows系统不能发挥全部的性能
