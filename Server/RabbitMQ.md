...
### 运维
1. 安装：依赖erlang，单独安装
1. 启动：./sbin/rabbitmq-server -detached(后台方式)
1. 管理：rabbitmqctl
   - status、cluster_status
   - stop：关闭erlang节点
   - start_app、stop_app、reset：针对raMQ程序
   - list_exchanges、list_bindings、list_queues [name type durable auto_delete]：后俩为是否持久化、是否自动删除
1. 集群
...
### 原理
1. 集群内部利用 Erlang 提供的分布式通信框架 OTP
### wiki
1. AMQP：Advanced Message Queue，高级消息队列协议，是面向消息的开放协议，RabbitMQ是其开源实现
1. 其他队列产品：老牌ActiveMQ、阿里的RocketMQ、Kafka