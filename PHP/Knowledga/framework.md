...
1. 操作
   ...
   - info：yac整体状态
1. 原理
   - 利用了mmap匿名文件映射，实现同族(父子)进程安全的内容共享
     1. mmap
        - 认识：linux提供的将文件映射到内存地址的方式，即将文件或者虚拟对象映射到进程内存地址空间。就可以用指针操作内容，操作系统自动回写脏页面到文件磁盘上，不用其他write系统调用，可实现进程间文件共享
          1. 同族进程会指向同一个匿名文件，不同族的指向不同，保护不受外部进程干扰
          1. 操作系统引用计数，进程结束自动回收
             - 所以yac运行条件：同族进程，至少有一个进行在运行
        - 分类
          1. sysvipc：通过物理内存共享，
          1. mmap：匿名映射模式/指定文件，不涉及具体文件，避免文件的创建和打开，很显然只能用于同族进程
          1. 
   - 需加锁，因为可能写一半的时候被读走
1. 其他
   - apc、apcu
...
1. 认识：像流水线一样处理用户请求，模块耦合低、扩展性高。运用了组件化开发、依赖注入、接口编程
   - 是一个比较重的框架，作为api使用可能性能无法做到极致。lumen是基于laravel组件构建官方支持的微框架，为更快的执行速度
...
1. 给出压测报告
1. 其他
   - 提供自拼sql查询的orm，因为他们有很多特殊的sql用orm拼不出来 
1. 方案：支持两种orm？
   - yii
     1. 支持度不高，更新慢
   - medoo
     1. 超级简单，代码小，一般的组合sql可以满足，复杂查询也可以
     1. 更贴近pdo，更易于直接调用pdo
     1. 无链式调用，where条件混杂字段当中，开发舒适度一般，需要自己做读写分离
   - doctrine
     1. 中文文档少，代码多，面向对象的sql语法，不够敏捷，感觉性能不咋地
     1. 都有9年前的代码
   - think-orm
     1. 无active record，一个DB类到底
     1. 各种使用条件超全
     1. 支持断线重连、长连接、sql性能分析、分布式事务
1. 特性
   - 长连接
   - 断线自动重连
   - 自动故障切换，节点选择
