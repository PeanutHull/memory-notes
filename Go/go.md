...
1. 变量
   - 认识：var、:=
     1. 类型在变量名后边，避免了类c的含糊不清的定义(放前面容易有歧义，因为指针符号*和类型可以组合，也可以和变量名组合，很灵活，容易有歧义)
     1. 默认类型推导
     1. 支持多变量同时操作
   - 操作分类：二者几乎完全相反，一个全局注重声明，一个局部方便使用。[](../images/go/var_=.jpg)
   - 类型分类
     1. 值类型：声明默认分配内存
     1. 引用类型：slice、map等
        - 认识：声明 + 分配内存用于存放值
   - 组成
     ...
     1. 作用域
        - 组成：局部(花括号内)、全局(函数外)、形式参数(函数定义中)
        - 特点
          1. 局部和全局可以重名，局部变量的改变不会改变全局变量，局部变量的优先级高于全局变量
   - 举例
    ```go
    // var
    var i int
    ...
    var i,j = 1,true        // 自动类型推导
    var (                   // 批量操作
        i int = 1
        j float32
    )

    k := 3                  // 短变量声明运算符：短声明 + 类型推导
    ```
   - 最佳实践
...
1. 应用相关
   - fmt
     ...
     1. 格式化参数：记住常用即可，特殊的查手册
        - %v   使用值的默认格式进行打印，结构体时只打印值、不打印字段名
        - %+v  结构体时添加字段名，其他和%v一样
        - %#v　相应值的Go语法表示
        - %T 　相应值的类型的Go语法表示
...
1. 配合其他语言
   ...
   - cgo
     ...
     1. 原理：先由编译器识别出import "C"的位置，然后在其上边的注释中提取c代码，最后调用c编译器进行分开编译
     1. 使用
...
1. 编译
   ...
   - 编译：一个package只能有一个main，否则build不过
     ...
     1. `go build`
        ...
        - 条件编译
          1. 认识：构建约束，即编译标签，和编译条件`-tags`相同字符的才编译在包中
             - 必须在在文件最顶部(package之前)，之后一个空行；约束可以出现在任何文件中
             -  *_GOOS、*_GOARCH、*_GOOS_GOARCH结尾的隐式包含构建约束
          1. 语法
             - 旧：`// +build condition`
               1. 语法比较别扭：多个条件之间，空格表示OR、逗号表示AND、叹号(!)表示NOT，一个文件可以有多个+build，关系是AND
               1. 只允许是字母数字或_
             - 新：`//go:build condition`，v1.17
               1. 语法用标准布尔表达式 && || ! ()
          1. 应用
             - `// +build ignore`：不编译某文件
             - `//go:build linux`：这个文件只在GOOS=linux时才参与编译
             - `//go:build !linux`：这个文件非GOOS=linux时才参与编译
        - 交叉编译
