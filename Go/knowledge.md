   ...
   - 最佳实践：![](../images/go/rpc_struct.png)
     ...
     1. 流量管控
        ...
        - 其它
          ...
          1. 服务预热：循环更新
     1. 应用可观测性
        - trace：追踪，把请求从api网关、服务层各层调用节点进行串联，可用来分析请求过程中服务、资源相关调用情况、调用瓶颈，强弱依赖等
        - log：日志，应用级相关日志，对访问、超时、内部异常等需要有日志覆盖
        - metric：指标，例如每秒请求数、错误数、超时数、异常数等等（可覆盖应用、接口、方法等统计维度）
     1. 微服务管理后台：查看、控制等
...
1. parnurzeal/gorequest：http请求库
   - 简单、功能丰富，链式调用
1. goreplay
   - 认识：开源网络监控工具、流量回放工具，可以实时记录TCP/HTTP流量，支持把流量记录到文件或es实时分析，也支持流量的放大、缩小，还支持频率限制
...
1. temporalio/temporal：提供容错分布式工作流引擎，即使在进程崩溃后也能从断点恢复执行。其事件溯源架构确保状态永不丢失。如不同系统间分布式的库存扣减一致性难题，跳出了db的事务
   - 是分布式解决方案的一种
#### 字符串
...
1. swagger和go的配合
   ...
   - 生成文档：`swag init`
   - done
1. google/wire：go的通过静态分析代码生成依赖注入DI代码的工具
   - 作用
     1. 大型的、依赖关系复杂的、需要高可测试性的、模块化的Go项目，如微服务、大型单体项目，提供类型安全的依赖管理，同时减少手动初始化的错误风险
     1. 避免了运行时反射带来的性能损耗，同时保证了类型安全
   - 方式：编译时生成代码
     1. 定义依赖关系：使用`wire.NewSet`定义依赖关系
     1. 生成代码：使用`wire.Gen`命令生成依赖注入代码
   - wiki
     1. facebookgo/inject：使用运行时反射的自动依赖注入容器，可以减少硬编码的依赖，使得组件更容易解耦合
        - 只在服务启动时，运行时反射有一定性能开销
        - 运行时才能发现依赖问题
        - 反射使代码难以追踪
### 语法
#### chan的应用
