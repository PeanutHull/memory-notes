...
1. 代码编译期插桩
   - 认识：实现了与Java字节码增强同等的便利性。这种方案最终生成的是一个自包含、高性能的二进制文件，将“增强代码”与业务代码深度融合，既保证了功能，又做到了对运行时的“零侵入”和“零损耗”。如在net/http包的RoundTrip函数入口插入监控代码
     1. 不同于java运行时的动态性，即可以在类加载的间隙“偷天换日”修改指令。而Go是一门静态编译语言，直接生成为机器码，没有留给运行时修改的空隙，实现从“运行时修改”转变为“编译时注入”
     1. 实现复杂，需要深入理解Go编译工具链，目前只有OpenTelemetry等少数项目在生产级探索
   - 步骤：在两个阶段进行操作
     1. 预处理：Preprocess，分析依赖关系并选择稍后应该使用的规则
     1. 代码注入：Instrument，根据规则生成代码，并将新代码注入源代码
     1. 继续编译：修改完AST后，Go编译器会继续它的工作（类型检查、代码生成、链接），最终产出一个已经包含了所有监控能力的、可直接运行的二进制文件
   - 使用
     1. -toolexec：拦截编译过程，在生成机器码前修改函数体，插入跳转代码（蹦床）
### 源码阅读方法
1. 方法
