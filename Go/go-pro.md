...
1. 认识
   - 内存分配
     1. 内存什么时候栈分配什么时候堆分配：主要取决于变量的作用域和类型。局部变量和函数参数在栈上分配，这些变量的生命周期通常与语句块的范围一致；而全局变量和动态分配的内存会在堆上分配，如malloc和new需要手动管理free、delete释放
   - 内存回收
   - 内存整理
...
1. GC
   ...
   - 基本算法
     1. 标记-清除法
        - 认识：从根对象出发，根据对象之间的引用信息，一步步推进直到扫描完毕整个堆并确定可回收的对象，由标记阶段和清除阶段构成。Go、Java、V8、JS的实现都是
        - 步骤
        ...
        - STW：Stop the World，为了避免GC的过程中新修改的引用关系到GC的结果发生错误需要进行STW，会影响程序性能，通过写屏障尽可能缩短STW的时间
          1. 对大哈希表非常要命，如4千万堆上对象，GC的扫描过程超过4秒
          1. 垃圾收集器中的两个“停止世界”阶段，其中goroutine会停止
        - Write Barrier：写屏障
     1. 引用计数法：每个对象自身包含一个被引用的计数器，当计数器归零时自动得到回收。因为此方法缺陷较多，在追求高性能时通常不被应用。Python、OC的实现都是
     1. 标记-复制法
   - 延伸算法：对三种基本算法的组合
     1. 标记-压缩法
     1. 保守式GC
     1. 分代回收法：按照对象生命周期长短划分不同的代空间，生命周期长的放入老年代，短的放入新生代，不同代有不同的回收算法和回收频率
     1. 增量回收法：三色标记法
1. 并发读写的顺序
