...
1. 命令
   ...
   - 重定向
     ...
     1. xargs：给命令传递参数的过滤器，捕获一个命令输出，传递给另外一个命令，一般和管道一起使用。因为很多命令不支持管道来传递参数
        ...
        - 实例：`find . -name "*.c" | xargs grep -Hn  | sort > result.txt`：查找文件中包含xxx的按照文件名排序并写入文件
...
1. 查看内容
   ...
   - head/tail：输出文件头/尾
     1. -n：行数
     1. -f：tail不停显示
     1. `tail -n +1000 xx.log`：从1000行开始显示以后的
     1. `tail -n 1000 xx.log`：显示文件的最后1000行
     1. `tail -f xx.log | grep -rn "xxxx"`：显示过滤后的最新写入的数据
     1. `tail -n +1000 xx.log | grep -rn "xxxx"`：显示过滤后的1000行以后的数据
...
1. 网卡
   ...
   - 分类
     1. eth0：目标ip是外网ip时使用
     1. lo：loopback接口，特殊的网络接口，用于目标ip是本地环回地址的，如127.0.0.1
     1. virbr0：
   - 操作
