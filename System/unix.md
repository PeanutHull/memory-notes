...
1. io引擎
   - 分类
     ...
     1. 存储映射
        - mmap
#### 高级IO
1. 认识：io模型的演进，其实就是时代的变化，倒逼着操作系统将更多的功能加到自己的内核而已
   - 阻塞read：网卡到内核缓冲区阻塞、内核缓冲区拷贝到用户缓冲区阻塞，内核态状态为：未就绪、已就绪、已完成
     1. 小把戏：用户态多线程read。实现了并行
   - 非阻塞read：如果未就绪就直接返回-1，还在存在系统调用、存在内核缓冲区拷贝到用户缓冲区的阻塞
   ...
   - io多路复用
     1. select：内核来遍历这个文件描述符的数组，既做到一个线程处理多个客户端连接（文件描述符），又减少了系统调用的开销
        - 细节
          1. 数组大了，拷贝到内核(可优化为不拷贝)，成本大
          1. 完成后需用户遍历(可优化为只返回给用户就绪的文件描述符，无需无效遍历)
          1. 内核仍然通过遍历的方式检查文件描述符的就绪状态(可优化为红黑树)
     1. poll：和select主要区别是去掉select只能监听 1024 个文件描述符的限制
     1. epoll：内核保存文件描述符集合，通过异步io事件唤醒
        - 方法
          1. epoll_wait()：阻塞调用epoll_wait获取就绪的fd
        - 触发模式
          1. 水平触发：Level Triggered LT，缓冲区中有数据发出通知，关心缓冲区数据
             - 对于读操作：只要缓冲内容不为空，返回读就绪
             - 对于写操作：只要缓冲区还不满，返回写就绪
          1. 边缘触发：Edge Triggered ET，缓冲区状态发生变化发出通知，关心缓冲区变化，如可读/可写、数据到达/发送、应用进程对相应的描述符进行EPOLL_CTL_MOD修改EPOLLIN/EPOLLOUT事件时
             - 一定要循环读取数据，直到缓冲区中的数据全部读取完成，一次性将数据取出
1. 阻塞io
   - read后：进程进入阻塞态
   - 系统产生中断：即io完成，进程进入就绪状态
   - 等待cpu调度分配时间片来执行，否则重新进入就绪状态
1. 非阻塞io
   - 使用：两种方式指定
...
1. 四大模型
   ...
   - 同步非阻塞：调用者发起io操作请求，询问io操作状态，如果未完成，则立即返回；如果完成，则返回结果。io操作的过程需要等待执行完成才返回结果
     1. 如使用同步调用方式调用非阻塞的方法，既不会阻塞代码的执行，也不会同步返回
