### 认识
1. 哲学
   - 小即是美：每个程序只应该做好一件事情，并且做好
   - 组合原则：协调去完成大而复杂的任务
   - 一切皆文件：尽可能将所有资源视为文件。文件、目录、设备甚至网络连接都可被视为文件，这简化了系统的接口和编程模型
   - 可移植性比效率更重要
   - 所有程序都是数据的过滤器
   - 尽可能地榨取软件的全部价值
1. unix架构：cpu、memcory、devices -> kernel -> applications
   - 操作系统的本意是做到让你简单可依赖，让你尽量把它当成一个黑盒。你想要一个字节，它就给你一个字节，但是自己默默干了许许多多的活儿
1. 网络
   - tcp连接数
     1. client：发起连接请求时，除非绑定端口，通常会让系统选取一个空闲的本地端口，所以可用端口最多只有65535(0特殊，不能用)
     1. 每个socket占用内存在15~20k之间，每一个tcp连接都要占一个文件描述符
1. 应用
   - 同时读写：`open(path, O_RDWR|O_CREAT|O_TRUNC, mode);`，creat需要开关两次实现
   - 进程崩溃时临时文件的确保删除：进程运行，open临时文件后，就unlink，等崩溃时就会删除
   - 获取命令行参数：getopt、getopt_long
   - for循环的写法
     1. for (i=0; i<5; i++)
     1. for (i=0; argv[i] != NULL; i++)
     1. for (dirst=readdir(dir); dirst; dirst=readdir(dir))
   - realloc可创建恰好的数组，如路径
...
1. 概念：cpu对n内存以外的设备进行读取，如硬盘和网卡。调用recv作为例子从套接字上接收一个消息，因为是一个系统调用，所以调用时会从用户进程空间切换到内核空间运行一段时间再切换回来，网络数据到达并且复制到用户进程空间或者发生错误时返回
   - 既不会像基本io模型中一直在accept()阻塞等待，也不会导致无法处理实际到达的recv()连接请求或数据
1. 认识：io模型的演进，其实就是时代的变化，倒逼着操作系统将更多的功能加到自己的内核而已，![avatar](../images/io_compare.png)
   - 阻塞read：网卡到内核缓冲区阻塞、内核缓冲区拷贝到用户缓冲区阻塞，内核态状态为：未就绪、已就绪、已完成
     1. 小把戏：用户态多线程read。实现了并行
   - 非阻塞read：如果未就绪就直接返回-1，还是存在系统调用、存在内核缓冲区拷贝到用户缓冲区的阻塞
     1. 进步点：可在一个线程完成多个fd操作
     1. 缺陷：需要多次的系统调用查询是否就绪
     1. 小把戏：单独线程遍历fd数组检查是否就绪。省了线程，没省系统调用
   - io多路复用
     1. select：内核来遍历这个文件描述符的数组，既做到一个线程处理多个客户端连接（文件描述符），又减少了系统调用的开销
        - 数组如果变大，拷贝到内核(可优化为不拷贝)，成本高
        - 完成后需用户遍历(可优化为只返回给用户就绪的文件描述符，省掉无效遍历)
        - 内核仍然通过遍历的方式检查文件描述符的就绪状态(可优化为红黑树)
     1. poll：和select主要区别是去掉select只能监听 1024 个文件描述符的限制
     1. 信号驱动io
     1. epoll：内核保存文件描述符集合，通过异步io事件唤醒
1. 信号驱动io
   - 认识：和异步io一样靠信号通知和回调函数实现，信号驱动io是数据准备好了，没有拷贝前发出信号通知；异步io更彻底，是已经把数据从内核空间拷贝到用户空间了再通知
     1. 调用sigaction注册信号函数，等内核数据准备好的时候系统中断当前程序，执行信号函数(这里为recv)，还是同步IO
1. io多路复用
   - 场景解释：有100万个客户端同时与一个服务器进程保持着TCP连接。而每一时刻，通常只有几百上千个TCP连接是活跃的(事实上大部分场景都这种)。如何实现这样的高并发？在select/poll时代，服务器进程每次都把这100万个连接告诉操作系统(从用户态复制句柄数据结构到内核态)，让操作系统内核去查询这些套接字上是否有事件发生，轮询完后，再将句柄数据复制到用户态，让服务器应用程序轮询处理已发生的网络事件，这一过程资源消耗较大，因此，select/poll一般只能处理几千的并发连接
   - 理解：解决非阻塞io带来的不成功问题，构造描述符列表，调用函数即阻塞，当某描述符准备好io时函数返回，那么确知read/write函数不会阻塞
   - 认识：一个进程监视多个描述符，一旦某个描述符就绪(一般是读写)能够通知程序进行相应操作。但select，pselect，poll，epoll本质上都是同步io，因为都需要在读写事件就绪后自己负责读写，也就是说这个读写过程是阻塞的，而异步I/O则无需自己负责进行读写，异步I/O的实现会负责把数据从内核拷贝到用户空间
   - select/pselect：指定变动条件的描述符列表、等待时间，准备好的描述符数量、3个条件(读写异常)的哪些描述符准备好
     1. select：![avatar](../images/select_io.png)
   ...
   - epoll：epoll模型是Linux2.6以上内核高性能网络I/O模型，只支持pipe/网络产生的fd，不支持文件系统产生的，即Reactor模式(能够高效处理大量并发连接，通过事件驱动的方式来处理io操作)，红黑树管理，log(n)复杂度
     ...
     1. 触发模式：一次和多次就绪通知的区别
        - 水平触发：Level Triggered LT，默认，缓冲区中有数据发出通知，关心缓冲区数据
          1. 对于读操作：只要缓冲内容不为空，返回读就绪
          1. 对于写操作：只要缓冲区还不满，返回写就绪
        - 边缘触发：Edge Triggered ET，高速，缓冲区状态发生变化发出通知，关心缓冲区变化
   ...
   - wiki
     1. Reactor模式：用于处理多个并发输入事件，通过将事件的处理与分发分离来实现高效的事件驱动编程，提高性能
1. 记录锁
...
##### 同步和阻塞
1. 阻塞
   - 阻塞io：![avatar](../images/block_io.png)
     1. read后：进程进入阻塞态：recv默认参数，一直等数据直到拷贝到用户空间，这段时间内进程始终阻塞
     1. 系统产生中断：即io完成，进程进入就绪状态
     1. 等待cpu调度分配时间片来执行，否则重新进入就绪状态
   - 非阻塞io
     1. 让recv不管有没有获取到数据都返回，轮询(往返用户进程空间)调用recv查看，可以做其他的事情，只有是检查无数据的时候是非阻塞的
     1. 使用：两种方式指定
        - 获得描述符时：open，指定O_NONBLOCK
        - 已经打开的描述符时：使用fcntl
1. 同步
   - 同步io：以上阻塞和非阻塞都是，因为数据到达时依然要等待复制数据到用户空间
   - 异步io
     1. 认识：当描述符准备好可以io时，用信号通知
        - 不同版本有不同的实现，可移植性
        - 无法分辨多描述符的信号通知。SUSv4解决了这种局限性
     1. 分类
        - System V：异步io信号是SIGPOLL，调用ioctl
        - BSD：通用异步io信号是SIGIO，SIGURG通知进程网络连接上的带外数据到达如tcp，调用fcntl
        - POSIX：使用aio控制块描述io操作，如aio_read/aio_write/aio_fsync/aio_error/aio_return/aio_suspend/aio_cancel/lio_listio
          1. aio：只支持Direct I/O（文件操作）
          1. io_uring：kernel在5.1加入，新归宿，适合长连接
        - Windows
          1. iocp：libuv在windows的异步方案
     1. 应用：假设一项工作有两个计算部分，一个io部分，io占用时间比计算多得多，同步式io想要实现高并发必须开多个线程。而使用异步单线程即可胜任
1. 同步和阻塞
   - 造成的原因：网卡和磁盘离CPU太远，数据获取太慢
   - 理解
     1. 同步、异步：消息通知机制，是否是别人来完成io的操作然后来告诉你。POSIX定义同步io操作为导致进程阻塞直到io完成的操作，反之则是异步io
        - 从编码调用方式看，是同步等待，还是异步回调
   ...
   - 四大模型
     1. 同步阻塞：最直观的处理方式，简单但可能效率低下。例子：标准的文件读写操作。等待io操作完成再返回。io操作的过程需要等待，io操作执行完成后返回结果
        - BIO：同步且阻塞，遇到io操作cpu占用并等待，可通过线程池改善
     1. 同步非阻塞：不常见，询问io操作状态，如果未完成，则立即返回；如果完成，则返回结果。io操作的过程需要等待执行完成才返回结果
        - 如使用同步调用方式调用非阻塞的方法，既不会阻塞代码的执行，也不会同步返回
        - NIO：同步非阻塞，都注册到多路复用器上，一个线程处理多个连接，逻辑部分是单线程模式，是异步io，轮询到有i/o请求时才启动一个线程处理
     1. 异步阻塞：相对少见，可能出现在特定的设计模式中。等待io操作完成再返回。io操作的过程不需要等待，操作完成后通过通知或回调获得结果
     1. 异步非阻塞：常见于现代高效能的应用程序中，询问io操作状态，如果未完成立即返回；如果完成，则返回结果。io操作的过程不需要等待，操作完成后通过通知或回调获得结果
        - AIO：异步非阻塞，一个有效请求一个线程，由os完成再通知服务器应用启动线程处理。用于连接数目多且连接比较长，充分调用os参与并发操作
### 进程
...
1. 相互的理解
   ...
   - 多线程、多进程
     ...
     1. 多线程：同时运行，可以直接通信
1. 守护进程
   - 认识：生存期长，系统引导装入时启动，系统关闭时停止，没有控制终端后台运行，就是长时间提供服务呗。系统的如kswapd、flush、sync_supers、jbd
   - 实现
     1. 规则
        - umask的注意
        - 调用fork，然后使父进程exit，则继承了进程组id，保证子进程不是组长进程
        - 调用setsid创建新会话，则成为新会话的首进程，成为新组长进程，没有控制终端
        - 当前工作目录改为根目录，防止占用挂载的文件系统
        - 关闭文件描述符
        - 打开/dev/null，使其具有文件描述符0124
     1. 惯例
        - 配置：启动时读取一次配置文件，需要捕捉SIGHUP信号然后重新读配置，存放在/etc目录
        - 日志记录：调用syslog，函数有openlog/syslog/closelog/setlogmask
        - 使用锁文件：name.pid
        - 自动重启：/etc/inittab设置respawn记录项
        - 单实例：任何时刻只运行一个副本，使用文件和记录锁lockfile提供互斥机制，如需要排他访问设备
     1. 编写：
        - 变为一个守护进程：P375
        - 单线程守护进程捕捉SIGHUP并重读配置文件：P384
#### 信号
...
1. 线程同步
   ...
   - 同步机制
     ...
     1. 自旋锁：申请锁时一直处于忙等(自旋)阻塞状态，会耗费cpu，不是休眠(互斥量)，用于锁持有时间短，线程不希望在阻塞和唤醒上花费太多成本，用户和核心态的转换。可用于非抢占式内核、实时调度中，会阻塞中断，不要有自旋锁的情况下使用可能会休眠的函数，会增加时间。有些互斥会自旋一小段时间才休眠
...
1. cas：Compare and Swap, 比较并交换，这是一个原子操作
1. 组合理解
...
1. linux父子进程写同一个文件
   - 会产生怎样的竞争问题
     1. 数据竞争：如果父子进程没有进行适当的同步且同时尝试写入文件，会导致数据竞争。即两个进程的写操作互相干扰，导致文件中的数据不一致或损坏
     1. 文件指针竞争：父子进程使用共享的同一个文件描述符，也会同时共享这个文件描述符的文件指针，一个改变文件指针的位置，可能导致数据写入位置错乱
     1. 原子性问题：较大字节的写操作系统可能会拆分为多个步骤来完成。因此两个进程的写操作可能会交错执行，导致数据混乱
   - 解决方案
     1. 文件锁：使用文件锁机制（如flock或fcntl）来确保一个进程在写入文件时，其他进程无法进行写操作
     1. 信号量或互斥锁：使用信号量或互斥锁来保护写操作，确保只有一个进程在任意时间点执行写操作
     1. 使用各自独立的文件描述符
##### 进程间通信
1. 理解：缩写IPC
1. 管道
...
1. sleep
   - 认识：基于系统调用nanosleep，使当前进程挂起一段指定的时间，期间不占用CPU资源，中断信号或时间到了后将进程的状态恢复为TASK_RUNNING，并重新将进程添加到运行队列中，等待调度器重新调度
     1. 会将进程状态设置为TASK_INTERRUPTIBLE（可中断的睡眠状态），并将进程从运行队列中移除，使得cpu可以去执行其他任务
### wiki
1. 其他
   - linux的基石是unix内核，基于unix的基本特点、posix、单独的unix规范标准实现
   - libc/libc.a：泛指c函数库，包括头文件和基本c库，最初由c发明者写，后来移植到多个平台，有了多个版本。包含常用的数据结构和处理方法，如数组(长度可变),单(双)向链表,hash表,队列,关系，处理方法如字符串/标准输出(g_print等)/错误输出/日志记录，还有事件循环/线程/io操作等，是ansi c函数库
   - glibc：c运行库，是gun发布的libc库/gun c函数库，是linux最底层的api，包括了unix通行的标准，各发行版linux都用，标准c库还有uclibc、klibc、libc，glibc用得最多，glib是gtk+的基础库
   - 编译apue.h：gcc a.c -l apue，.a文件是静态库文件，在链接阶段，会将汇编生成的目标文件.o与引用到的库一起链接打包到可执行文件中称为静态库，是一组目标文件的集合。-l指定头文件目录，-std=c99指定c方言
1. 历史
   - 1.0：1994年，第一个官方的稳定版内核
   - 2.0：1996年
   - 2.2：1999年
   - 2.4：2001年
   - 2.6：2003年，大量的性能改进，更多硬件的支持
   - 3.0：2011年
   - 3.10：2013年（长期支持版本），增加了对Btrfs文件系统的改进，更好的网络性能
   - 4.0：2015年，引入了实时内核补丁功能Live Patching
   - 4.4：2016年（长期支持版本）
   - 4.9：2016年（长期支持版本），最长时间的支持版本之一，加强了安全性和网络功能
   - 4.14：2017年（长期支持版本）
   - 4.19：2018年（长期支持版本），性能和安全性改进
   - 5.0：2019年
   - 5.4：2019年（长期支持版本）
   - 5.10：2020年（长期支持版本），改进了文件系统和网络性能
   - 5.15：2021年（长期支持版本）
1. 问题
   - 多进程模式下的文件原子读写
   - 避免死锁的代码再好好理解下，11.6.2
   - 互斥量的递归非递归
   - 线程和信号的代码，12.8
   - 线程和fork理解，12.9
   - 守护进程的收到信号重读配置的信号处理部分不理解，P383
