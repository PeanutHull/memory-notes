...
1. 阻塞
   ...
   - 非阻塞io
     1. 让recv不管有没有获取到数据都返回，轮询(往返用户进程空间)调用recv查看，可以做其他的事情。只有是检查无数据的时候是非阻塞的，数据到达的时依然要等待复制数据到用户空间，因此还是同步io
     1. 使用：两种方式指定
        - 获得描述符时：open，指定O_NONBLOCK
        - 已经打开的描述符时：使用fcntl
1. 同步
   - 同步io：以上阻塞和非阻塞都是，因为需要用户等待I/O操作完成，并接收返回的内容
   - 异步io
...
1. 同步和阻塞
   ...
   - 四大模型
     1. 同步阻塞：调用者发起io操作请求，等待io操作完成再返回。io操作的过程需要等待，io操作执行完成后返回结果
        - BIO：同步且阻塞，遇到io操作cpu占用并等待，可通过线程池改善
     1. 同步非阻塞：调用者发起io操作请求，询问io操作状态，如果未完成，则立即返回；如果完成，则返回结果。io操作的过程需要等待执行完成才返回结果
        - 如使用同步调用方式调用非阻塞的方法，既不会阻塞代码的执行，也不会同步返回
        - NIO：同步非阻塞，都注册到多路复用器上，一个线程处理多个连接，逻辑部分是单线程模式，是异步io，轮询到有i/o请求时才启动一个线程处理
     1. 异步阻塞：调用者发起io操作请求，等待io操作完成再返回。io操作的过程不需要等待，操作完成后通过通知或回调获得结果
     1. 异步非阻塞：调用者发起io操作请求，询问io操作状态，如果未完成立即返回；如果完成，则返回结果。io操作的过程不需要等待，操作完成后通过通知或回调获得结果
...
### wiki
1. 其他
   - linux：Linux的基石是unix内核，基于unix的基本特点、POSIX、单独的UNIX规范标准实现
   - libc：泛指C函数库，包括头文件和基本C库libc.a，最初由c发明者写，后来移植到多个平台，有了多个版本。包含常用的数据结构和处理方法，如数组(长度可变),单(双)向链表,hash表,队列,关系，处理方法如:字符串,标准输出(g_print等),错误输出,日志记录，还有事件循环,线程,IO操作等，是ANSI C函数库
     1. glibc：c运行库，是GNU发布的libc库，是linux最底层的api，包括了unix通行的标准，各发行版Linux用的就是glibc，是GUN C函数库，Linux下面的标准c库还有uclibc、klibc、libc，glibc用得最多，glib是GTK+的基础库
