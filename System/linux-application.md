...
1. 火焰图
   - 认识：基于perf结果产生的svg图片，用于展示cpu调用栈。顶部有平顶就可能有性能问题
     1. perf：linux原生提供的性能分析工具，返回cpu正在执行的函数和调用栈，通常抽样频率是99Hz，结果不易阅读，火焰图就有了
...
1. 存储类型
   ...
   - 文件
     ...
     1. 场景
        ...
        - 日志存储
   - 对象
     1. 认识：多台服务器内置大容量硬盘，安装上对象存储管理软件，对外提供读写访问功能
...
1. 存储协议
   ...
   - 分类
   - nas
     1. 认识：network attached storage，网络附属存储，实现大容量、高可靠的网络协议、可共享的进行数据传输的专用数据存储服务器，存储效率高于服务器，成本更低
        - 支持多种协议，如NFS、CIFS、FTP、HTTP
   ...
   - san：存储区域网络
   - das：直接附加存储，直接附加到单个计算机的存储，是nas的前身
   - nfs：unix常用
   - SMB：Service Message Block，服务器消息块协议，大多数为window，该协议在windows中被称为“Microsoft Windows 网络”，基于CIFS开发，被称为CIFS/SMB协议
     1. CIFS: Common Internet File System通用网络文件系统，windows主机之间共享的协议，samba实现了这个协议
   - AFP：运行macOS的apple设备的专有协议
   - s3
   - swift
1. 文件共享服务
   - NFS
     1. 认识Network File System，网络文件系统，通过网络让不同的机器、不同的操作系统可以共享文件，让pc将网络中的NFS服务器共享的目录挂载到本地的文件系统中，使用非常便利。一般用来存储静态数据，工作在内核模式下
        - sun公司研制的unix表示层协议，基于UDP/IP协议，主要采用RPC实现，因为nfs不同的功能都会使用不同的程序来启动，用rpc沟通对应的网络端口来交互
        - 权限需要搭配NIS，Network Information Services，以用户组和/etc/passwd来判断权限(和本地权限一样)
   - Samda
     1. 认识：基于SMB协议的开源软件，linux上共享文件和打印机等资源，是cs型，client(linux)访问server(windows)的资源，两个系统文件共享
   - FTP
   ...
   - AFS
     1. 认识：Andrew File System，分布式文件系统，用于管理分部在不同网络节点上的文件，采用安全认证和灵活的访问控制提供一种分布式文件系统，卡内基梅隆大学开发，结构与NFS相似，但有所增强
   - DFS
     1. Distributed File System，分布式文件系统
   - 比较
     1. 分布式文件系统和网络文件系统的区分要分清
1. 分布式文件系统
...
1. 虚拟机网卡模式
   ...
   - 桥接模式：相当于局域网内创立了一个单独主机，需要手动分配ip地址和子网掩码，和真实主机在同一个网段，可以互相ping通
