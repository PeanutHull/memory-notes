...
1. wiki
   - 云服务模式
     1. IaaS：基础设施即服务
     1. PaaS：平台即服务
        - apaas：应用程序平台即服务，低代码平台即服务
     1. SaaS：软件即服务，面向用户
        - 优势
          1. 可配置性：每个租户的功能都可以不同。租户数据隔离、不同租户管理和授权
          1. 高伸缩性：租户大数据量时如何支撑，如何水平扩展和垂直扩展
          1. 高可用性：系统稳定，24小时服务
        - 用户设计
...
1. 能力
   - Web攻击防护
     1. 常见漏洞攻击防护：sql注入、xss攻击、命令执行等OWASP Top 10攻击
     1. 0day漏洞虚拟补丁
     1. 恶意扫描与探测防护：如SqlMap等自动化扫描工具的探测行为
   - 流量管理与访问控制
     1. CC攻击防护：通过频率限制、人机验证（如滑块、js挑战）等手段缓解资源耗尽型攻击
        - js挑战：验证访问者是人还是机器
     1. 精准访问控制（ACL）：基于IP、URL、地理区域等条件设置黑白名单，精细化管理流量
        - 全局态势感知：在别的地方发现是坏人，大家都防御
        - ip信誉库：基于大数据的恶意ip库
   - 行为分析：基于行为的攻击检测
     1. 恶意爬虫管理：
   - 可视性与运维
   ...
   - 扩展安全与合规
     ...
     1. 合规性支持
### 边缘计算
1. 认识：应用部署在边缘端，离设备更近，类似设备和总控中间用作计算、整理的角色，如智驾、交通大数据。就是在云服务商在运营商机房里放几台自己的机器
   - 用于：超低时延、不稳定连接、数据移动性等，并且需要实时决策、本地化算力
   - 边缘计算AI用的多
