...
1. Disruptor
   - 认识：是线程之间用于消息传递的一种内存消息队列，类似Kafka，在Apache Storm、Camel、Log4j2等中广泛应用
     1. 可以说性能最高，比ArrayBlockingQueue高一个数量级
   - 实现
     1. 采用RingBuffer、AvailableBuffer两个结构，批量写入、读取n个队列单元，只加一次锁，极大降低锁粒度
1. JMS和AMOP
   - 理解
...
### flower
1. 认识：异步非阻塞的反应式编程框架
   - 分布式开发：将flower的service远程部署到service容器中，向服务中心注册后启动，应用程序可对这些service进行流程编排，得到分布式非阻塞的微服务系统
     1. 不同的是异步的，通过流程编排进行服务调用，而不是接口形式的阻塞调用
1. 反应式系统
   - 特点
     1. 即时响应：非阻塞
     1. 回弹性：自我修复，不宕机
     1. 弹性：自动伸缩，调整自己处理能力或者调整进入系统的请求数量
     1. 消息驱动：通过消息传递完成服务流程