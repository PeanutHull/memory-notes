...
1. 数据表
   ...
   - 索引
     ...
     1. 索引类型引擎支持情况
        - InnoDB：b+tree，即除了空间列是rtree，其他的都是b+tree。hash索引由引擎根据情况自动创建，不能人为干涉
        - MySIAM：b+tree、rtree
...
1. 索引优化和命中
   - 优化措施
     1. 不能有null：有null的列不会包含在索引中
     1. 使用短索引：提高查询速度，节省空间和io
     1. 尽量选择区分度高的列作为索引：区分度的公式是COUNT(DISTINCT col)/COUNT(*)，表示字段不重复的比率，比率越大我们扫描的记录数就越少，唯一索引是1
     1. 尽可能扩展索引，而不是增加索引
     1. 不在列上运算：因为每个行要运算所以索引失效
     1. like使用：最左原则，%aa%不使用索引，而aa%使用
     1. 索引列排序：一个查询语句只用一个索引：where用了，order by就没法用
     1. 长字符串使用前缀索引：通过select left(column,n)不断增加n查看重复行的减少，以达到最佳选择性
     1. 合理添加联合索引：多列经常查询/排序使用联合索引
   - 不使用索引情况
     1. !=、not in、<>不使用索引，范围查询可能用到索引如>、in等
     1. or前后条件都有索引，整个语句才使用索引，否则推荐用union
     1. 函数运算
     1. 存了数字的字符串丢掉引号，不能用int存数字
   - 聚集索引或非聚集索引的选择
1. 锁
### 原理
