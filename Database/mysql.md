...
1. 事务
   ...
   - 并发事务
     1. 产生的问题：https://www.cnblogs.com/snsdzjlz320/p/5761387.html，以下都是1没commit，或者只是普通的读数据
        ...
        - 不可重复读：默认，同一事务的多个实例在并发读取数据时，会看到同样的数据行。1没commit看到已经2已经修改的值前后时间是一样的
   ...
   - undo保证事务的原子性，redo保证持久性。
1. MVCC：对于支持行锁的事务引擎，进行数据库的并发控制，把数据库的行锁与行的多个版本结合起来,只需要很小的开销,就可以实现非锁定读,从而大大提高数据库系统的并发性能。提供基于某个时间点的快照。总是可以提供与事务开始时刻相一致的数据,而不管这个事务执行的时间有多长
