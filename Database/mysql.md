...
1. 锁
   - 分类
     1. 基于数据操作
        - 共享锁：`lock in share mode;`，读锁，读读可以并行
        - 排他锁：`for update`，写锁，读写不能并行，写写不能并行
          1. 锁级别：有主键或索引行级别，无则表级别
          1. 注意
             - 仅适用于InnoDB，必须在事务中执行
             - 普通select写锁拿不到，for update读写都拿不到，update/delete/insert自动加，select任何锁不加
     ...
     1. 基于范围
        ...
        - 间隙锁：范围查找自动锁定范围内所有行，不可以被其他事务读取/修改，防止幻读
        - 页锁：BDB被InnoDB取代，介于表和行之间，会死锁
     1. 基于逻辑
        - 悲观锁
