...
1. 数据类型
   ...
   - 字符串
     1. char：定长字符串，0~255字节，m最大保存长度，数据小于m空格填充，速度更快甚至快过varchar50%，用于很短字符串或长度接近的，经常变更的，char不容易产生碎片
        - utf8可以存储3个字节的字符，utf8mb4可以存储4个字节的字符，完全覆盖所有Unicode字符，所以根据不同编码类型，utf8mb4最多可以存储63个4字节的汉字
     1. varchar：变长字符串，m最大保存长度，超出截断并报警，使用1或2个字节前缀记录字符串长度
        - 根据实际存储的字符串确认长度，但是排序等操作是按照最大长度计算的，所以小点好
        - 最大不超过行大小，默认65535字节，如utf8最多2万个字符
        - 小于255个字符用1个字节作长度前缀，所以255是分界点
   ...
   - 日期和时间
     1. datetime
        - 认识：最通用、是绝对时间值、时区无关，有可变的最多6位默认0的毫秒位数长度
        - 形式：`YYYY-MM-DD HH:MM:SS`，支持DEFAULT CURRENT_TIMESTAMP和ON UPDATE CURRENT_TIMESTAMP
        - 范围：`1000-01-01 00:00:00 ~ 9999-12-31 23:59:59`
        - 大小：v5.6.4之前固定8byte，之后支持微秒，根据毫秒位数确定
        - 最佳实践
          1. 默认可以设置为`1970-01-01 00:00:00`
          1. `0000-00-00 00:00:00`的不合适在于
             - 有兼容性问题：v5.7及以上默认的sql_mode为不包含`NO_ZERO_DATE`和`NO_ZERO_IN_DATE`，不认为是有效值，否则报错
             - 逻辑上不合理：不是一个有效的时间
             - 会破坏索引
        - 历史
          1. v5.7默认记录毫秒值，v8.0不会
     1. timestamp：表示时间戳范围、自带时区属性、存储的是相对于utc的秒数，处理不同时区方便，值会随着服务器时区的变化而变化，有可变的最多6位默认0的毫秒位数长度
   ...
   - 其他
     ...
     1. json：与longtext大小差不多，不能有默认值，v5.7.8
        - 操作：查询、字段值提取
          1. 索引：json字段本身不能直接被编为索引，可以使用路径表达式对json文档中的标量值建立虚拟列，然后在虚拟列上建立索引
        - 实现原理
          1. 存储：以DOM的层次化的结构存储，使用utf8mb4字符集，是BLOB的子类在其基础上做了特化
             - 如每一级对象，存储的最前面为存放当前对象的元素个数，以及整体占的大小
1. 变量
   - 分类
...
1. 表
   ...
   - 分类
     1. 临时表：只有当前连接可见，关闭连接自动删除，如`create temporary table tableName ();`，show tables看不到该表
        - 系统自动临时表的情况：最佳实践是通过优化查询或索引策略来避免使用临时表，从而提高查询性能
          1. 排序sort、分组group by操作中数量超过一定大小后，查询优化器建立的临时表，当查询需要对结果进行排序/分组，而不能利用索引直接返回已处理好的数据时
   ...
   - 改变结构
     ...
     1. v8.0.12：instant算法，InnoDB秒级别快速增加列
        - 限制有
...
#### 基本操作
1. 查询方式
   - 关联查询：join on and，`join xx on x1.x=x2.x and y1.y=y2.y`
     1. 认识：都会生成一张中间临时表，再将这张临时表返回给用户
        - on条件是在生成临时表时使用的条件
        - where条件是临时表生成后，再对临时表进行过滤的条件
     1. 形式
        - inner join：内连接，二者无关系就不显示，同join
        - left/right join：外连接
          1. left：获取左表所有记录，即使on中的条件不真，右表显示为null
          1. right：获取右表所有记录，即使on中的条件不真，左表显示为null
...
1. 使用
   - 字段选择
     1. 有唯一性限制的
     1. 经常用于where查询的，考虑联合索引，一起查的话就不用回主表了
     1. 经常用于group by和order by的字段，索引里已经是排好序的数据了
   - 场景
     1. 非常小的表：全表扫描效率更高，有索引反而会拖慢更新
     1. 中大型：索引非常有效
     1. 特大型：索引代价增长，考虑分表
1. hash
...
1. btree
   ...
   - 特性
     ...
     1. 覆盖索引：指查询所需的全部字段都可以在索引中找到，而不需要回表查询主键索引，在联合索引中存在
        - 鉴定：explain中如果extra显示using index，则表示查询使用了覆盖索引
...
1. 事务
   ...
   - 常见问题
     1. 事务回滚不会回滚自增id的值。自增id一旦分配，即使事务回滚，也不会被回收重用
        - 自增id是在插入操作开始时分配的，而不是在事务提交时
        - 自增计数器存储在内存中，为了提高并发性能，回滚事务不会递减这个计数器
     1. 读顺序：trx_id解决
...
1. 事务的特性：ACID
   ...
   - 一致性：Consistency，所有操作在任何时候都保证一致，成功后所有操作才可见，比如前后一致，如唯一索引回滚后不能有重复键
...
1. 隔离性的隔离级别
   ...
   - 隔离级别：由低到高
     ...
     1. Read Committed(RC)：读已提交，读取自身版本和最新版本，以最新为主，不加锁读，本事务中可以看到其他事务提交的数据
        - 避免脏读，导致不可重复读、幻读
        - 读锁能被写锁升级，读写并行(写读不行)
     1. Repeatable Read(RR)：可重复读，默认，事务开始后其他事务的修改在本事务中不可见，但只针对修改不针对插入；同时会对读和读的范围加锁，新的满足查询条件的记录不能够插入(间隙锁)
        - 避免脏读/不可重复读/幻读
        - 幻读
          1. 解决方式
             - 快照读（普通select），通过mvcc解决幻读，不可解决其他事务相关范围插入
             - 当前读（select for update等），通过next-key lock（记录锁+间隙锁）方式解决了幻读，阻止其他事务在锁范围内插入，更好避免幻读
          1. 没完全解决幻读问题，对于修改的操作依旧存在幻读问题
            - 一个事务不加where更新，另一个事务插入，则第一个更新事务会将第二个插入事务的结果修改，且更新事务前后两次查询不一样了，发生幻读
            - 事务开启后，先快照读，然后其他事务插入，然后当前读时，两次查询不一样了，发生幻读
        - 读锁不能被写锁升级，读读并行
     1. Serializable：序列化，事务变为串行化顺序执行，从mvcc并发控制退化为基于锁的并发控制，只能读读并行，有写读不行，更别说写写，并发度急剧下降并增加死锁机率
        - 避免脏读/不可重复读/幻写
     1. Snapshot isolation：快照
   - 对应的问题
     1. 更新会相互覆盖从而丢失数据
     1. 脏读：A看到了B没commit的数据
     1. 不可重复读：本事务过程中可以看到其他事务提交了的数据，或者读到自己事务内写前后不一的数据
...
1. 分区
   ...
   - 最佳实践
     1. 腾讯云数据库使用规范：短期内业务达不到一个比较大的量级，建议禁止使用分区表。分区表主要用作归档管理，多用于快递行业和电商行业订单表，分区表没有提升性能的作用， 除非业务中80%以上的查询使用分区字段。
   - 分区交换
...
1. 大表
   ...
   - 风险
     1. 慢查询
     1. DDL操作
        - 建索引：v5.5之前锁表，之后不锁表但主从延迟，修改时间是要double的
1. 大事务
   - 定义：运行时间较长、操作数据较多的事务
   - 风险
     1. 锁定太多数据，造成大量阻塞、锁超时
...
1. 查询
   - 联表查询
     1. 最佳实践：小心使用，查看explain结果，可能在多个表的全表扫描后嵌套循环查询产生高昂的成本
     1. 查询方式：嵌套循环连接算法，驱动表会被访问一次，被驱动表可能会被访问多次。即驱动表的每一行，都会搜索被驱动表以找到匹配的行
     1. 连接查询总成本 = 单次访问驱动表的成本 + 驱动表扇出数(fanout) x 单次访问被驱动表的成本
        - 所以方法就是
          1. 一 尽量减少驱动表的扇出：要利用驱动表的索引缩小范围
          1. 二 对被驱动表的访问成本尽量低：被驱动表的数据要小
        - 如果3个表以上连接，就可能造成1x2x3=6，这种翻倍的查询成本
        - 不同的表作为驱动表最终的查询成本可能是不同的，也就是需要考虑最优的表连接顺序。内连接来说，驱动表和被驱动表的位置是可以互换的
   - limit：如果用到了索引就只会查一部分数据，如果索引上没查到想要的，就继续滚动继续往下找，也是会走索引。offset才会全查出来然后扔掉前边的，limit不会，使用id>xx来规避深offset问题
1. 分表分片
   - 认识
     1. 分表：可以将两种方式结合使用
...
1. 范式
   ...
   - 三范式
     1. 组成
        - 1NF：原子性，属性不可再分解
          1. 要根据具体情况判断是否可以再拆
        - 2NF：惟一性，不存在部分依赖，要求其他字段都依赖于主键。如学号、课程号、姓名、学分中，学分依赖课程，但不依赖学号，产生了部分依赖
...
1. 设计和使用
   - 数据类型
     1. 尽量使用更简单的类型，数据长度越短越好(更少存储内存空间、更快响应速度)
     1. 长数字使用string
     1. 用枚举代替常用字符串类型
     1. 日期
        - 不要用字符串，一是不好查，二是不支持时间函数
        - 使用int不如直接使用timestamp，还支持时区、存储量更小
   - 列设计
     1. 一定有主键，最好是自增，否则多次读写后磁盘上存储的更离散，产生更多随机io
     1. 增加create_time/update_time字段，用于数据归档/自定义差异备份
     1. 大数据字段独立表进行存储，提高表性能
     1. 名称不要和关键字碰撞
     1. 给文本字段留足余量
     1. 不能设置为可以为null
   - 查询
     ...
     1. 提高索引利用率
     1. 尽量用union代替子查询，以及union all代替union
     ...
     1. 包含not in和<>的子查询：将`select xx NOT IN (select xx)`优化为`LEFT JOIN xx WHERE b.xx IS NULL`
   - 事务
...
1. 索引优化策略
   ...
   - 使用原则
     ...
     1. 优化器会评估，有可能放弃使用索引
   - 查询   
     1. 索引列上使用表达式或函数：因为每行要运算所以索引失效
     1. 参数类型不匹配产生隐式转换，会导致不能使用索引，如字符串列不加引号识别为数字
     1. 索引最左前缀原则，联合索引中间的字段查询无法使用索引
     1. like：以通配符开头%aa%不使用索引，而aa%使用

     1. 非范围查询!=、not in、<>：不使用索引，范围查询可能用到索引如>、in等
     1. 使用is null或is not null不使用索引
     1. or：前后条件都有索引才会使用索引，否则用union连接查询结果

     1. 数据分布不均：某值频率过高超过约30%时，优化器可能选择全表扫描而非索引
     1. on、using子句上有索引，否则全表
     1. 特定情况下使用hash索引优化查询
     1. 使用JOIN时类型不匹配无法使用索引
   - 排序
     1. 使用order by与where条件不匹配不能使用索引
       - 索引的列顺序和order by子句顺序完全一致
       - 索引中所有列的方向(升降序)和order by子句完全一致
     1. 使用distinct或group by与索引不匹配不能使用索引
   - 维护
     1. 查找未被使用过的索引
        ```sql
...
1. 认识
1. explain
   - 理解：sql语句分析，将过程和索引等信息列出来。explain中type为index就是用到了
   - 使用解析
     1. select_type：查询类型，越复杂的成本越高
        - simple：最简单的，不包括union或子查询
        - primary：包含复杂的子部分，如union或子查询
        - union：出现在union之后
        - subquery：子查询的第一部分
     ...
     1. filtered：存储引擎返回的数据经过滤后，满足查询条件数量的百分比
     1. extra：问题解决提示信息
1. sql度量
...
1. 数据库扩容的方式
   ...
   - 业务定制：历史数据作归档库，将用户操作用哈希操作落到业务库和归档库上
