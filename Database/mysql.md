...
1. ACID
   - 概括undo保证事务的原子性，redo保证持久性，隔离性通过锁和mvcc实现，其他通过日志
   - MVCC：Multi-Version Concurrency Control 数据多版本并发控制，对于支持行锁的事务引擎，进行数据库的并发控制，把数据库的行锁与行的多个版本结合起来,只需要很小的开销,就可以实现非锁定读,从而大大提高数据库系统的并发性能。提供基于某个时间点的快照。总是可以提供与事务开始时刻相一致的数据,而不管这个事务执行的时间有多长
1. 索引
   - 分类
