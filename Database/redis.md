...
1. 发布订阅
   - 认识：pub/sub，可以实现多播的消息通信模式，发送订阅模型，是原子的
     1. 消息丢失：
     1. 无持久化：未连接或消息在网络中丢失，redis不会重试或存储消息，挂掉的消费者重新连上期间的消息会被丢弃，重启redis消息不会持久化，因为相当于一个消费者都没有
     1. 无确认机制：发布者无法知道是否被接收
   - 使用
