...
1. 缓存常见问题：![avatar](../images/redis_tips.webp)
   ...
   - 缓存数据一致性保证的数据更新方式
     1. 更新数据库 + 更新缓存
        - 认识：并发情况下两种类型操作，2n次操作一旦交叉就会出现数据不一致，不推荐
          1. 命中率最高，可加锁牺牲写性能
        - 方式
          1. 先更新数据库 + 后更新缓存
          1. 先更新缓存 + 后更新数据库
     1. 更新数据库 + 删除缓存：延迟双删就是更新数据库前后删除两次缓存，更新后还要睡眠一段时间再删缓存，目的是让读请求完成读数据库然后写缓存的一套操作，没有根本解决冲突，只是尽可能保证一致性
        - 读：先读缓存，没有则读数据库，再set回缓存(可加锁防止数据库雪崩)。缓存可设置有效期
          1. 具体方案：数据并发竞争预热方案，休眠时间应该改为范围随机。![avatar](../images/redis_data_update.webp)
        - 写：先del缓存，再写数据库————读的时候再初始化缓存就行，不能反过来，否则缓存可能有问题
1. 性能监控
   - qps、读写峰值
   - 客户端数
   - cpu
