...
1. 内存淘汰
   - 策略
     1. 周期淘汰：贪婪策略，默认100ms：抽取过期的比例超过四分之一就继续执行
        - 方式：根据过期时间的先后、随机、LRU、LFU
     1. 惰性淘汰：被访问时检查过期时间
   - 设置
     1. noeviction：不淘汰，数据满了写请求返回错误
     1. volatile：针对过期的，四种方式
     1. allkeys：针对所有的，三种方式，没有根据过期时间
#### 应用
1. 队列
   - 即时队列
     1. 措施
   ...
   - 方式分类
     ...
     1. AOF
        - 认识：append only file，增量日志，以redis协议的格式来保存，新命令会被追加到文件的末尾，后台可进行重写压缩大小
          1. 只记录修改的命令
          1. rdb搭配aof使用可极大降低数据丢失可能性，甚至可以不要rdb，启动时恢复数据aof比rdb方式慢
          1. aof默认30秒刷盘，如果操作系统异常则数据丢失，可设置append fsync同步硬盘的时机
        - 实现
     ...
     1. 混合持久化：v4.0，aof不再是全量的日志，而是rdb之后的增量日志，即rdb完成后清空aof文件
        - 既有rdb快速恢复的好处，又有aof只记录操作命令的高性能
1. 单机：单点故障隐患，性能瓶颈
1. 主从
