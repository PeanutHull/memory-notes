...
### 应用
1. Bloom Filter
   - 认识：布隆过滤器，可以理解为不怎么精确的set结构，v4.0
   - 操作
...
1. 缓存策略
   - 认识
     ...
     2. 策略就是数据的访问模式，即如何写入和读取的
     3. 不同方式的应用场景和优缺点不同
     4. 大多数关系数据库存储引擎(如InnoDB)其内部默认启用write-back缓存策略。首先写入内存，最终刷新到磁盘
   - 方式
     1. cache-aside：旁路缓存模式。db为主，缓存为辅
        - 读：先查询缓存是否存在，存在直接返回；不存在查询db，然后将结果写入缓存
        - 写：先更新db，然后直接删除对应缓存
        - 特点
          1. 适合读多写少，弱一致的场景
          1. 缓存模型可以和db模型不同
          1. 会出现缓存和db数据不一致的情况：更新完db没来得及删缓存时
        - 缺点
          1. 第一次读取会穿透：提前预热机制，并发竞争预热时休眠时间应该为范围随机。![avatar](../images/redis_data_update.webp)
          1. 写多导致缓存命中率低：删缓存变为改缓存，强一致场景用锁保证；弱一致场景缓存时间设短，不加锁
     2. read/write through：读写穿透模式。缓存为主，db为辅
        - 读：和cache-aside一样，但是缓存模型和db模型相同，写缓存通常由中间件独立完成，有一定自动化，最近更新和访问的数据总是在缓存中可用
        - 写：先查询缓存是否存在，不存在，直接写db；存在，先更新缓存，然后更新db
        - 特点
          1. 适合读多写少，强一致的场景
          1. 可以保证一致性
          1. 引入了额外的写db延迟
     3. write behind：write back 后写模式。缓存为主，db为辅
        - 读写：和read/write through一样，但是异步批量更新数据库
        - 特点
          1. 适合读写都频繁，但不是数据强烈强一致的场景，但是需要保证异步写的完整性，否则数据不一致，适合如浏览量、点击量
### wiki
1. 历史
   - 2009年，开源
