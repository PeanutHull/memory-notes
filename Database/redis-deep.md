### 整体设计
1. 认识
   - 单进程单线程，命令是串行执行。包含核心处理逻辑的单线程设计机制以及多路复用机制，结合多进程处理其他逻辑
     1. 认识
        - 主要指redis的网络io和键值对读写是由一个线程来完成的，其他如持久化/异步删除/集群数据同步等，由额外线程执行
        - epoll机制主要用在了客户端的连接处理和数据收发、发布订阅模式的处理、主从复制同步等
        - 避免了并发访问控制的锁等待开销
     1. 特点
   ...
   - 架构组成
     ...
     1. 可扩展集群支撑模块：分片机制
   - 单线程和多进程的应用
     1. 单线程：处理所有客户端的请求，数据的读写操作，事件驱动处理网络连接、命令解析、命令执行
     1. 多线程：v6.0用于网络读写
     1. 多进程：rdb后台生成、aof重写
1. 数据类型
   - 全局哈希表
...
1. 内存
   ...
   - 内存占用
     1. 认识
        - 删除了数据，内存可能还没有释放给操作系统，占用率下不来
        - 内存碎片产生的原因是redis使用了内存管理器jemalloc，在一段连续内存中删除数据，要么过小无法利用
          1. 内存管理器分配内存都是按照固定大小给的，不是按照实际大小
