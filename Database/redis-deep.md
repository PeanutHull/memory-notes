...
1. 字典：dict，最常见的复合数据结构
   ...
   - 代码
    ...
    ```
1. 压缩列表：ziplist，是一块连续的内存空间，元素之间紧挨着存储
   - 使用范围：zset、hash元素个数较少时
   - 结构
...
1. 双向链表 linkedlist
1. 快速列表
   - 背景：早期版本list元素少时用ziplist，多时用linkedlist
   ...
   - 代码
    ...
    ```
1. 跳跃列表
   - 认识：skiplist，算法复杂度O(log(n))，搭配hash_table用于SortedSet
...
1. 紧凑列表
   ...
   - 代码
    ...
    ```
1. 基数树
   - 认识：rax 有序字典树，按照key的字典序排列，支持快速地定位、插入和删除操作
     1. 易于理解，实现非常复杂
