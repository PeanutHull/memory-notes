...
1. Beam
1. HBase
   - 认识：面向列、高可靠、高性能，可伸缩，实时读写的分布式非关系型数据库，利用hdfs作为文件存储系统，支持mr程序读取数据，可以存储非结构化、半结构化、结构化数据，可以存储大量小文件
     1. 海量存储：上百亿行*上百万列
     1. 准实时查询
     ...
     1. 支撑离线分析型应用
   - 架构体：zk->master->多个RegionServer->hdfs
   - 组成
...
1. Hive
   - 认识：提供大量结构化数据检索的功能的数据仓库。用hdfs存储、MapReduce进行计算。是sql解析引擎，将sql转换为mr job在hadoop中执行，hive的表就是hdfs的目录/文件。写sql就可以执行hadoop任务，降低使用门槛
     1. 基于hdfs
     1. 可用于数据提取转化加载ETL
     1. 支持类sql查询
...
1. hive
   - 认识：基于hadoop之上的数据仓库，常用于大数据量离线分析场景
     1. 可以存储、查询、分析存储在HDFS中的大规模数据
     1. SQL Parser将用户SQL进行解析
     1. Compiler将HiveQL转换Executor
     1. Executor是最小处理单元
     1. 每个Executor代表HDFS的一个操作或一道MapReduce作业
     1. 计算引擎现在可以支持Spark替代MapReduce
