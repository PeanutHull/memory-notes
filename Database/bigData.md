...
1. 学习要点
   ...
   - 简单使用
1. 数据处理框架
   - 认识：让大规模数据处理技术变得更易用。框架是实际处理数据的组件，引擎是一系列组件
   - 分类
     1. 仅批处理
        - Hadoop
     1. 仅流处理
     ...
     1. 混合
        ...
        - Flink
   - 处理类型
     1. 批处理：静态数据集，计算完成后返回结果，常用于历史数据进行分析，适合需要访问所有数据才能完成的工作。依赖存储从而慢和处理成本低(廉价硬件)、极高的缩放潜力
        - 有界：数据是有限集合
        - 持久：持久存储
        - 大量
     1. 流处理：数据没边界，适合近实时处理需求，关注一段时间内变化趋势的数据
     1. 实时ETL：抽取、转换、加载
   - 工作分类
     1. 离线数仓：有hive、kylin、hadoop，离线一般在hive里，高并发查询会用mysql，不能聚合和复杂sql
     1. 实时流式
1. wiki
...
1. MapReduce
   ...
   - 开发
     ...
     1. 编译打包后提交到hadoop运行
1. HBase
   - 认识：面向列、高可靠、高性能、可伸缩、实时读写的分布式非关系型数据库，kv类型的，利用hdfs作为文件存储系统，支持mr程序读取数据，可以存储非结构化、半结构化、结构化数据，可以存储大量小文件。适合非结构化数据存储的数据库，随机访问，实时读取
