...
1. 原理和使用技巧
#### 分布式数据库
1. TDSQL：腾讯云的分布式数据库，对外是整个数据库，现代的分布式存储都是计算和存储相分离的，如三部分组成。感觉就是包了好多个mysql弄个分布式
   - 管理节点：存储元数据
   - 计算节点：sql引擎，根据元数据决定将请求发往后面的哪些存储节点、处理分布式场景下的全局自增字段等逻辑
   - 存储节点：多个set，每个set由agent+主库+n个从库等组成
     1. raft保证强一致性、强同步复制，Master接收到业务请求后，等待其中一个Slave应答成功后才会返回成功
     1. Agent可以管理具体的Mysql实例，做到无损升级等
        - 自动容灾切换：如果Master节点故障，它的Agent会上报它的异常给MetaServer。接着Master会降级成为Slave。再从剩下的Slave中选举一个出来
