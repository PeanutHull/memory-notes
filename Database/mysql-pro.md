...
1. MVCC
   - 认识：Multi-Version Concurrency Control 数据多版本并发控制，一致性数据快照，即非锁定读。指的是维护一个数据的多个版本，主要用来解决数据库并发读写问题。当多个用户同时访问数据时，每个用户都可看到一个在某时间点之前的数据库快照，并能无阻塞地执行查询和修改操作，而不会相互干扰
     1. 做到读写、写读不冲突，极大提升了性能，就剩下写写的冲突了
        - 读的并行度能达到或超过读未提交，而隔离级别很高，跟序列化相差不多
     1. 大多数情况下可以代替行级锁，并提供事务开始时相同的数据，不管事务执行的时间有多长
        - 读取数据时InnoDB几乎不用获得任何锁，每个查询都通过版本检查，只获得自己需要的数据版本，从而大大提高了系统的并发度
     1. 针对读多写少场景优化，写多时可能增加系统的成本
        
   - 特点
     1. 通过维护数据的多个版本(多时间点的快照)，并结合读视图，有效地支持了“读已提交”和“可重复读”两种隔离级别
   ...
   - 读的分类
     1. 按照方式
        - 当前读：读最新版本，加锁
        - 快照读：属于一致性读，读可见且历史版本，基本不加锁，如读事务开始时之前的版本，可以确保是已经保存的，或者事务自身修改的
          1. 每个读都通过版本检查，只获得自己需要的数据版本，从而大大提高了读的并发度
     1. 是否具有一致性：前后是否一致
        - 一致性非锁定读：非加锁
        - 一致性锁定读：加锁
          1. for update：x锁
          1. lock in share mode：s锁
     1. 是否锁定：是否加锁
   - 实现
     1. 认识：为数据库中的每一行创建多个版本，通过版本链找到旧数据，每个版本对应一个当时事务的快照，即对比数据行的事务id与事务的id是否兼容
     1. 组成
        - 版本链：行中的三个隐式字段
          1. DB_TRX_ID：最近修改的事务id，插入这条或最后一次修改的事务id
          1. DB_ROLL_PTR：回滚指针，配合undoLog指向这条记录的上一个版本
          1. DB_ROW_ID：隐藏主键，就是那个表结构没有指定主键时自动创建的
        - undoLog：undo会记录每一次的update，并且新插入的行数据会记录上一条undo日志的指针，所以可以通过这个指针找到上一条记录，这样就形成了一个版本链
        - consistent readView：一致性读视图，为实现一致性读，确定事务中哪些版本的行记录是可见的，![](../images/db/consistentReadView-trxId.webp)
          1. creator_trx_id：创建该ReadView的事务ID
          1. m_ids：所有活跃事务(已开始未提交)的ID列表
          1. min_trx_id：m_ids数组中最小的事务ID
          1. max_trx_id：事务ID的最大值加一，即下一个要生成的事务ID
     1. 原理
        - 当事务进行读操作时，实际上是从这些版本中选择一个符合其隔离级别的视图来读取，而不是直接读取最新的数据，可以读取旧版本的数据。这样就能够在不加锁的情况下实现事务之间的隔离
          1. 确定读的顺序：使用逻辑时间戳确定读写顺序，保证时间先后顺序即可，不是真正意义时间的描述，对应物理时间戳
        - 每次事务更新数据时，都会生成一个新数据版本(row)，并更新DB_TRX_ID和DB_ROLL_PTR
          1. 旧数据版本保留了，新数据版本中有信息可以拿到
          1. 写时将数据复制一份，以版本号区分，copy on write
        - 当事务开始时会为其创建一个ReadView，怎么判断某个事务在当前事务是否可见呢？4种情况
          1. 如小于min_trx_id，表示这个版本是已提交的事务或者是当前事务自己生成的，数据可见
          1. 如大于max_trx_id，表示这个版本是由将来启动的事务生成的，肯定不可见
          1. 如在min_trx_id和max_trx_id之间，包括两种情况
             - 若row trx_id在数组中，表示这个版本是由还没提交的事务生成的，不可见
             - 若row trx_id不在数组中，表示这个版本是已经提交了的事务生成的，可见
   - 问题
     1. mvcc如何实现读已提交、可重复读隔离级别的？
        - 区别在于生成readView的时机不同，都是先生成一个readView
          1. 读已提交：每次读取数据前都生成一个readView，后续所有的select都复用这个readView。这样就能保证每次读取的数据都是最新的。每个事务看到的是已经提交的最新数据
          1. 可重复读：在第一次读取数据时生成一个readView，这个readView会一直保持到事务结束，这样可保证在事务中多次读取同一行数据时，读取到的数据是一致的。因为可重复读要求更高，要求前后读一致
1. 内部XA事务
...
1. 日志
   ...
   - undolog
     1. 认识：回滚日志，记录修改前的数据，用于回滚/撤销事务的更改，记录修改前的数据、与当前操作相反的操作日志
        - update/delete存放数据旧记录
        - insert记录新数据行的PK(rowid)
   - redolog
     1. 认识：重做日志，记录修改后的数据，存储了事务执行过程的日志，能够重新执行(重做)这些事务中的操作，确保事务的更改能够被恢复执行，保证可靠的事务，innoDB独有
