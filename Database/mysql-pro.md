...
1. Buffer Pool
   - 认识：innoDB的重要组件，是内存缓冲区，用于缓存数据、索引和其他innoDB的内部数据结构，以减少对磁盘的访问并提高数据库性能。会将数据以页page为单位保存在内存中，当查询请求需要读取数据时优先从bufferPool获取数据，避免直接访问磁盘
     1. 整页加载：即便只访问了一行数据的一个字段，InnoDB也会将整个数据页加载到bufferPool中，以便后续的查询
     1. 定期刷脏页：修改数据时优先修改缓存页面变为脏页。脏页不会立刻写回到磁盘，会定期刷
     1. 改良的LRU算法管理缓存页：将最近最少使用的数据移出缓存，为新数据腾出空间
   - 最佳实践
     1. 合理的bufferPool大小：通常为物理内存的70%
     1. 主要看命中率评估其作用，参数`Innodb_buffer_pool_read`，`Innodb_buffer_pool_read_requests`请求数
1. wiki
