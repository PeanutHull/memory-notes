...
### 索引
1. 分类
   - 聚集索引：clustered，聚簇索引，叶节点包含所有数据，多行检索快
   - 非聚集索引：非聚簇索引，有二次查询问题
1. 特点
   - innoDB的每一个表都会有聚集索引
     1. 如果表定义了PK，则PK就是聚集索引
     1. 如果表没有定义PK，则第一个非空unique列是聚集索引
     1. 否则，InnoDB会创建一个隐藏的row-id作为聚集索引
   - 叶子节点存储行记录(row)，InnoDB索引和记录是存储在一起的，而MyISAM的索引和记录是分开存储的
1. 索引
   - 类型分类
     1. 主键索引：Cluster形式，所有数据放在叶子节点(Leaf Node)，还存储下一个叶子节点的指针
     1. 普通索引：Secondary Index，只存放当前索引的数据和主键id，所以要获取本普通索引之外的数据需要找到主键id，然后去主键索引上拿数据
   - 属性分类
