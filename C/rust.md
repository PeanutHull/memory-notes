...
1. 成为开发操作系统的关键特性
   - 零开销抽象：Rust的设计哲学之一是“零成本抽象”，意味着Rust的高级抽象不会引入运行时开销，这对于性能敏感的操作系统开发至关重要
   - 内存安全：Rust通过所有权、借用和生命周期等机制在编译时强制执行内存安全，这减少了内存泄露和悬垂指针等错误
   - 类型安全：Rust强类型系统有助于在编译阶段捕获更多的错误，进一步增强代码的稳定性和可靠性
   - 并发编程：Rust的所有权模型自然支持无数据竞争的并发编程模式，这使得编写多线程代码更加安全和容易
1. go的缺点
   - 深度优化困难，用c和汇编比go快
