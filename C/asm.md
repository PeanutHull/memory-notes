...
1. 指令
   ...
   - 举例
     ...
     1. ret：返回
1. 内存布局
   - 认识：用户内存和操作系统内存是分开的，依次往上为操作系统内存栈，自由分配区，堆，数据区，代码区，数据区存常量变量，![avatar](../images/memary.jpg)
   - 堆
   ...
   - 栈
     1. 认识：stack，从高往低、先入后出(FIFO)、以栈帧为单位处理的连续的内存地址
...
1. plan9
   - 规则
     1. 没有r或e的前缀
   - 伪寄存器
     1. pc：即意义和作用都是指令寄存器，x86是ip，amd64上是rip
