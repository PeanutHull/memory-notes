...
1. 变量：程序可操作存储区的名称，变量类型决定存储的大小和布局，如枚举、指针、数组、结构、共用体等
   ...
   - 分类
     ...
     1. 静态：
   - 存储类别：使用存储说明符，用于定义变量、函数的作用域，在类型前面书写
     1. auto：只能修饰局部变量(即自动变量)，即仅存在于当前代码块{}中，进模块时生成退模块时消亡，自从所有非全局变量默认为auto以来很少使用，无处不在
     1. static：在函数调用时保持局部变量的存在，不需要每次进入、离开作用域时创建和销毁(场景之一是重复调用函数不会被重新赋值)，只能被函数内访问。用于全局变量和函数时，作用域为当前文件。静态变量无法定义为寄存器变量，静态变量存在静态区
     1. extern：提供一个全局变量的引用，对所有文件可见，用来在另一个文件中声明一个全局变量或函数。修饰变量时可定义可声明，修饰函数时只能是定义
