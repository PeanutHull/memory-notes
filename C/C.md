...
1. 备注
   - 数据类型
     ...
     1. char：ascii码是0~127，7位就可以保存
     1. 类型的存储大小：系统位数有关，64位为主
     1. 进制：默认10，0开头为8，0b为2，0x为16
     1. signed/unsigned(无符号)：用于修饰所有基本类型，unsigned默认为unsigned int
...
1. 空指针：或NULL，不和任何数据的有效地址对应，用于文件结尾或未按预期，是一个地址，通常占4字节
...
1. 变量：程序可操作存储区的名称，变量类型决定存储的大小和布局，如枚举、指针、数组、结构、共用体等
   ...
   - 分类
     ...
     1. 静态
   - 存储类别：使用存储类别说明符，用于定义变量、函数的作用域，在类型前面书写
     1. auto：只能修饰局部变量(即自动变量)，即仅存在于当前代码块{}中，进模块生成退模块消亡，自从所有非全局变量默认为auto以来很少使用，无处不在
     1. static：函数调用时能够保持存在，每次进入、离开作用域时不会创建和销毁(场景之一是重复调用函数不会被重新赋值)，只能被函数内访问。作用域为当前文件。静态变量无法定义为寄存器变量，静态变量存储在静态区
     1. extern：提供一个全局变量的引用，对所有文件可见，修饰变量时可定义可声明，修饰函数时只能是定义
     1. register：定义存储在寄存器中而不是内存中的局部变量，修饰寄存器变量，访问速度最快，意味着变量最大尺寸等于寄存器大小，不一定存储在寄存器中，取决于硬件和实现的限制，也许被占满，尽量放入寄存器，是暗示不是命令，不能使用&运算符(没内存位置)。寄存器只用于大量频繁的操作、需要快速访问的变量，如计数器
     1. _Thread_local：c11新增，？？？
...
1. 函数
   ...
   - 定义：`type funcName()`
   - return返回值：main函数中的return可以表示错误类型传递给系统
...
1. 符号
   ...
   - 存储类型说明符：const、volatile、restrict
...
1. 表达式
   - 左值：指向内存位置的表达式，可出现在左右边，变量是左值
   - 右值：存储在内存中某些地址的数值，不能被赋值，只能出现在右边
1. 注释
   - //：单行注释
   - /* ... */：多行注释，不能嵌套注释
1. 语法：大小写区分，执行顺序从上往下(后边定义的函数前边不能用，用函数声明解决)、从左至右，结束符;
1. 名称空间
   - 理解：c使用名称空间标识程序的各部分，即通过名称来识别。作用域是名称空间一部分
   - 共享名称空间：结构、联合、枚举标记共享相同名称空间，和普通变量的空间不同。相同作用域中，变量和标记的名称可以相同，不提倡，c++不允许
### 应用
1. 输入输出：都是依赖内置的函数，在stdio.h头文件中声明
   ...
   - 输入：从命令行
        ...
        - gets：`char *gets(char *s)`，从stdin读取一行到s所指向的缓冲区，直到一个终止符或EOF，存在缓冲区溢出(访问未分配内存导致可能覆盖内容)和安全问题
        - fgets：一参可以指定为字符串数组来获取输入
        - scanf：从stdin读取输入，并根据提供的format来浏览输入，空格作为截止符
   - 输出
        - putchar：把字符输出到屏幕上，并返回整数，可循环使用
        - puts：`int puts(const char *s)`，把字符串s和换行符写入到stdout
        - fputs：不会追加换行符到stdout，需要stdout作为写入数据的参数
...
1. 文件：文件的本质是一系列的字节，c提供了访问顶层函数，也提供OS层面。提供了各种函数来按字符或者以固定长度字符串的形式读写文件，都是失败返回EOF
   ...
   - 写入
     ...
     1. fprintf：`int fprintf(FILE *fp, const char *format, ...)`，写入字符串，输出到标准错误流`fprintf(stderr, i)`
...
1. c编程
   - 基本策略：将源代码翻译为可执行文件(机器码)
     1. 将c用编译器转为目标代码文件，包含机器码，不能直接运行
        - 编译器：c的标准编译器不一定会采纳，其中之一是GUN项目下的gcc编译器，紧跟C标准的变动
...
1. 历史：为了编写UNIX操作系统而发明，以B语言为基础，是最广泛使用的系统程序设计语言，驱动，其他mysql也是c
   ...
   - C99标准
     ...
     1. 识别两种浮点数类型：实浮点数和复浮点数，头文件complex.h
