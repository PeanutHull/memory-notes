...
1. 数据类型
   ...
   - 布尔：_Bool，无符号整数类型，只有0和1
   - 派生：数组、结构、联合、枚举、指针、函数、void
1. 数组
   - 认识：固定大小的相同类型元素的顺序集合，由连续的内存位置组成，数组大小不可修改
   ...
   - 使用
    ...
    array[0]
    ```
1. 字符：char，''，1byte。ascii码是0~127，7位就可以保存
   - 字符类型
...
1. 字符串
   - 认识：""，使用空字符\0终止的char类型的字符数组，本质存储整型，字符数量永远加1
    ```c
    // 定义字符
    char a = 'a';
    char a = 97;
    ...
    ```
1. 枚举
   - 认识：声明符号名称来表示整型常量，逗号隔开，标识符可有可无。实际enum常量是int类型
...
1. 结构体
   ...
   - 位域
     ...
     1. 使用
        ...
        ```
1. 联合
   - 认识：允许在相同的内存位置存储不同的数据类型，可以定义有多成员的联合，提供了一种使用相同的内存位置的有效方式。典型用法是存储无规律、事先不知道顺序的混合类型
     1. 联合类型的数组中联合大小相等，每个联合可储存各种数据类型
     1. 给不同的成员赋值会影响其他成员的值，同时只使用一个类型，点运算符表示正在使用的数据类型
   - 定义
    ```c
    union tag {                 # 联合模板
...
1. 特性
   ...
   - 布尔：C99，包含stdbool.c头文件后，可用bool表示_Bool，true表示1，false表示0，和C++兼容
   - 后缀：u无符号整数，l长整数。单精度常量：2.3f，双~：2.3，默认双
   - 符号：signed、unsigned。char默认signed，unsigned表示unsigned int
   - 字符串如果没有\0就是字符数组，\0的ascii码是0，\转义，\分行，空格连接，如`"1" "2" == "12"`，类似数组整个字符串是首字符的指针编译器可以使用内存的一个副本表示所有的相同的字符串，就是用一个内存地址表示
   - 数据类型转换
...
1. 变量：程序可操作存储区的名称，变量类型决定存储的大小和布局，如枚举、指针、数组、结构、联合等
   ...
   - 存储类别说明符
     ...
     1. 组成
        - static：每次进入/离开作用域不会创建/销毁(场景如重复调用函数变量不被重新赋值)，只能函数内访问，作用域为当前文件。无法定义为寄存器变量，存储在静态区
        ...
        - _Thread_local：c11，声明具有线程存储期，属于线程私有
        - typedef
...
1. 指针
   - 认识：是一个变量，值为另一个变量的内存地址，值都是十六进制数，不同指针类型是指指向的变量或常量的类型不同，可用于动态内存分配
     1. 间接寻址：保存指针的内存地址，定义 `int **ptr;`，赋值 `pptr = &ptr;`，获得内存地址上的值 `**pptr`
   - 使用
     1. 定义 `type *name;`
...
1. 日期时间
1. 内存管理
   - 认识：动态内存分配，需要大块内存时使用。c中静态对象、自动对象、动态分配对象(在内存堆或自由内存)存在不同区域
...
1. shell交互
   - 参数：通过main的参数获取，`int main(int argc, char *argv[]){}`，argc参数数量，argv[1]参数值，argv[0]存程序名称，参数用空格隔开，用单双引号包裹
   - 函数
     1. getchar：文件指针为标准输入
     1. putchar
