...
1. 数据类型
   - 分类
     1. 布尔：_Bool，无符号整数类型，只有0和1。C99包含`stdbool.c`后，可用bool表示_Bool，true表示1，false表示0，和C++兼容
     1. 基本：整数、浮点数
        - `char`：1byte，-128~127
        - `short`：2byte，前后3万
...
1. 字符串
   - 认识：""，使用空字符\0终止的char类型的字符数组，本质存储整型，字符数量永远加1
      - 字符串如果没有\0就是普通的字符数组，\0的ascii码是0
      - \转义，\分行，空格连接，如`"1" "2" == "12"`
      - 类似数组整个字符串是首字符的指针编译器可以使用内存的一个副本表示所有的相同的字符串，就是用一个内存地址表示
   - 实例
    ```c
    // 定义字符
    char a = 'a';
    ...
    ```
1. 数组
   - 认识：array，固定大小的相同类型元素的顺序集合，由连续的内存位置组成，数组大小不可修改
