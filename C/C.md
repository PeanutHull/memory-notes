...
1. 特性
   - 基础特性
     1. 大小写区分
     1. 执行顺序从上往下(后边定义的函数前边不能用，用函数声明解决)、从左至右
     1. 结束符;
   - 注释
     1. //：单行注释
     1. /* ... */：多行注释，不能嵌套注释
...
1. 变量：程序可操作存储区的名称，变量类型决定存储的大小和布局，如枚举、指针、数组、结构、联合等
   ...
   - 变量限定符：限制变量的使用方式
     ...
     1. volatile：声明编译器不会将限定的变量被外部代理改变，如硬件地址、其他程序、同时运行的线程中共享数据，用于编译器的优化，可放在cache中
        - 目的是禁止编译器优化读写操作
        - 并非为并发程序设计，不会保证访问的原子性，仅可以禁止重排序。可能不同线程访问时因为读写缓存(cpu的寄存器等)表现为不同的结果
        - MSVC中赋予其强制刷新缓存的语义，可以保证可见性，java中也可以
...
1. 日期时间
1. 内存管理
   - 认识：动态内存分配，需要大块内存时使用，分配在堆区，谁使用谁释放
...
1. 编译工具
   ...
   - makefile：类似于批处理的"脚本"文件，内含编译等指令，换个平台就需要重新编写
     1. 语法：目标+依赖+命令
     1. 目标比依赖的文件更旧、不存在，命令才会被执行，所以可适用于任意工作，不限于编程
     1. ![avatar](../images/makefile.jpg)
     1. 组成
        - 变量定义
        - 显示规则
        - 隐晦规则
