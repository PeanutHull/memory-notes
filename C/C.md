...
### 基本
1. 特性
   - C语言规定，未初始化的全局变量和局部静态变量的初始值为0，即没有占用任何空间，所以bss段不占用空间
1. 基础特性
...
1. shell交互
   ...
   - 函数
     ...
     1. printf：格式化打印
### 编译
1. 预处理器：CPP，C Preprocessor，不是编译器的组成，是编译过程一个单独的步骤，不做语法检查，就是文本替换。包含宏、文件包含、条件、编译
   - 命令：都以井号#开头，从第一列开始写
     1. #define、#undef：宏
     1. #include：文件包含
...
1. GDB
   ...
   - wiki
     ...
     1. 其他调试器
        ...
        - dlv：调试go
1. gcc
   - 认识：GNU Compile Collection，GNU编译器套装。包含C/C++/Java/Python/OC/Ada/Fortran/Pascal等编译器，能够为很多不同机器生成机器码。紧跟c标准的变动
   - 作用：源代码 —— 预处理器 .i —— 编译器 .s —— 汇编器 .o —— 连接器 —— 可执行二进制文件
     1. `gcc -E hello.c hello.i`：预处理阶段，预处理器cpp进行文本替换、宏展开、删除注释等简单工作
   ...
   - 使用
     ...
     1. `gcc -O2 -g -o p main.c sum.c`：
        ...
        - -o：目标文件名
   - wiki
     1. 库，即函数集合，存放头文件中的函数原型和对应的目标代码、链接过程的重定位信息
        - 静态库：以.a形式结尾，是.o文件的集合，在程序链接阶段使用，程序运行不再使用静态库，使得可执行文件变大
        - 共享库：以.so结尾，window叫动态库以dll结尾，程序运行中使用，文件小
     1. 编译器：转换为目标代码
1. 目标文件
   - 认识：二进制的包含一些section的描述文件，包含代码和数据，用于二进制文件、可执行文件、目标代码、共享库的转储。汇编器和链接器可以解析，分别会从两个角度解读
   - 格式
     1. ELF：Executable and Linkable Format，unix上主要的命名格式
   - 形式
     ...
     1. 共享目标文件：可被动态的加载和链接
   - 存储空间布局
     1. .text：代码段，正文段，cpu执行的机器指令部分，通常只读防修改。text、data、bss地址在执行时是连续的
     1. .data：初始化可读写数据段
     1. .bss：未初始化可读写数据段，仅是占位符，表示占用的字节数，不占用空间，通过节头表说明预留的空间，区分二者为了空间效率
