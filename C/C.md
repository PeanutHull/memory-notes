...
1. 计算机
   ...
   - 结构
     ...
     1. 冯诺依曼结构：数据和代码放在一起
   - 概念
     1. 系统自举：就是系统刚启动，自己把自己需要的代码载入，然后再执行更多的逻辑
     1. 用户空间、内核空间：内存划块，用户进程可以进入内核空间，相当于每个进程的虚拟4G内存，权限低不能访问系统内存和设备
     1. 时间
        - 时钟时间 ＝ 阻塞时间 ＋ 就绪时间 ＋ 运行时间
        - 运行时间 = CPU时间 = 用户CPU时间 + 系统CPU时间
        - 用户CPU时间：用户进程获得CPU后在用户态执行的时间
        - 系统CPU时间：用户进程获得CPU后在内核态的执行时间
        - 实际时间：多线程或者子进程情况下，并行执行可能小于cpu时间，io密集型存在等待可能大于cpu时间
     1. 时钟
        - 时钟周期：晶体振荡器在1秒时间内产生的时钟脉冲个数就是时钟周期的频率
        - 时钟滴答：通道0的计数器减到0值时产生一次时钟中断，即时钟滴答，计数器的初始值决定了滴答的时间间隔
