...
1. 继承
   ...
   - 定义：基类、派生类
    ...
    ```
1. 重载：指定多个定义
   - 函数重载：重载是扩展了函数/方法签名，从只认名字变为名字+参数类型，即同样的函数名称不同的形参，编译器会进行重载决策，决定合适的定义
     1. go认为重载有时是有用的，但在实践中也可能令人困惑和脆弱
   - 运算符重载：能使用自定义类型的运算符，重载的运算符是带有特殊名称的函数
     1. 形式：`ClassName operator+ ();`，关键字operator+ 符号
     1. 不可重载的运算符
        - .：成员访问运算符
        - .*, ->*：成员指针访问运算符
        - ::：域运算符
...
### 类库
1. yasio：轻量级的跨平台的异步的支持c++11的socket库，专注于任何客户端应用程序的asio(异步套接字I/O)
### wiki
