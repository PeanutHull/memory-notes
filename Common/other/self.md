第一性原理和归纳法
   - 第一性原理
     1. 逻辑基础：演绎推理（从原理到结论）
     1. 可靠性：理论自洽，但依赖初始假设的正确性
   - 归纳法
     1. 逻辑基础：经验总结（从现象到规律）
     1. 可靠性：依赖样本代表性，可能以偏概全
程序员素质
   - 求真求细节：`能跑就行，别动了`
   - 异常处理见真章
     1. 日志完善、日志量恰到好处
     1. 异常情况周到
   - 沟通方式
     1. 评估工期的姿势：`脱口而出两天`，`我先看看，脑子里过一遍全流程，要不要改表结构、要不要刷数据、`
     1. 描述bug的方式：`接口挂了，不知道为什么，昨天还好的`，`下午两点开始订单查询接口P99从200ms涨到2s，看了监控发现数据库连接池等待队列在涨，慢查询日志里多了一条全表扫描，怀疑是今天上午上线的那个新查询没走索引。`
     
   - 面对不会的问题
     1. 硬编：东拉西扯说一堆沾边不沾边的东西，试图用信息量掩盖理解的缺失
     1. 知道自己不知道，能在有限信息下做推理
   - 用微服务的方式写单体：注册中心用Nacos，网关用Gateway，配置中心、链路追踪一应俱全。打开代码一看：所有服务共用一个数据库，服务之间互相调用没有熔断，A挂了B跟着超时C跟着排队，一个接口慢拖垮整条链路。部署倒是分开部署了，但每次上线要同时发布四个服务才能保证兼容，发布顺序还不能错


### 工作
1. 如何查看自己写的笔记？因为笔记对于知识点的高度总结性，所以通过看笔记恢复对知识的掌握时，需要思考着看，需要遇到问题时进行思考再往下走的看
   - 需要先看一遍，比较耗费时间，不看怎么能知道？全都忘了
   - 看的时候会眼花，形成走过场，要用灵魂和知识网串起来
...
1. 设计步骤
   - 需求分析
     ...
     1. 服务器粗估
   - 领域设计
     1. 领域实体：需明确标出实体属性、核心方法
     1. 领域服务交互：需明确标出应用服务核心方法
   - 开发工作量预估
     1. 接口定义严格遵守API接口&文档规范
   - 工程质量
     1. 性能压测报告
     1. 单测覆盖度报告
     1. sonar代码质量报告
     ...
     1. 安全扫描
   - 运维
     1. 部署图：需明确标出模块在服务器上部署情况、以及域名、CDN等资源使用情况
     1. 降级方案
1. 绩效管理
   - kpi：考核，度量工具，面向稳定业务
