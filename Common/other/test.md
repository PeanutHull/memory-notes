...
1. 流量管理
   - 流量复制
   - 流量回放
     1. 认识：把线上的流量放到测试环境回放进行自动回归测试，回放后分析系统存在的问题
     1. 三个步骤：录制、回放、对比
     1. 组成
        - 线上服务：流量复制服务
        - 中间层：流量处理服务
        ...
        - 最终：结果对比
1. 测试工具
   - codecov：代码覆盖率检查工具，github在用
   - sonarlint：实时代码编写检查工具
   - sonarqube：代码质量检查工具
#### 分类
1. 分类
   - 业务测试
...
1. k6
   - 认识：开源的压测工具，使用js语音编写压测脚本，其内部使用go语言编写，使用Goja库作为js的解释器
   - 特点
     1. 适合api压测
     1. 支持http1.1、http2、websocket、sse
     1. 支持结果输出到influxdb、json、csv等
     1. 支持脚本录制
   - 实例
     1. 脚本编写
        ```js
        ...
        let res = http.get('http://your_host/api/v2/chat', {
            headers: { 'Accept': 'text/event-stream' },
            // body: JSON.stringify({ prompt: "你好" }), // 如需POST
        });
        check(res, {
        ...
        ```
     1. 运行：`k6 run script.js`
1. wrk：轻量级HTTP性能测试工具，支持自定义lua脚本，比ab好用？wrk支持http1.1，ab只支持http1.0，并且支持支持自定义lua脚本
    ```lua
    // wrk.lua文件
