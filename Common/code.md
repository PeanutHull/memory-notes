...
1. DDD
   ...
   - 好处
     ...
     1. 帮助拆分系统
   - 概念
     1. 实体：数据（属性）和行为（业务逻辑关系）的结合体，如博客
     1. 值对象：作为一个属性存放于一个实体内部，如置顶的博客
     1. 聚合：一组具有内聚关系的领域对象（包括实体和值对象）的集合，如博客基础信息
     1. 仓储：连接领域层和基础设施层的桥梁，一般将仓储接口定义放在领域层，仓储的具体实现放在基础设施层
   - 分层设计
...
1. 设计
   ...
   - 幂等设计：除了避免产生重复数据之外，还要求每次请求都返回一样的结果
     1. 数据库级
        ...
        - 加乐观锁version
        - 加唯一索引
        - 建防重表：新建单独的可以是多个字段唯一索引的表。防重表和业务表必须在同一个数据库中，并且操作要在同一个事务中
     1. 根据状态机：如订单状态的流转，原理同version，但是只应用在状态会变更的情况下
     1. redis分布式锁
     1. 获取token：先拿token，根据token决定请求是佛有效
### 软件
1. 建模与设计
