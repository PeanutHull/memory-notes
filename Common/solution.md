...
1. 策略
   ...
   - 失败告警
   - 日志
1. crontab：方便直接，大批量任务难以胜任，没有一套资源监控、执行监控、报警、日志体系
1. jdk自带
   - Thread类：只能执行周期性任务，不能指定时间点
   - Timer类：jdk的定时器工具，支持指定时间点，单线程执行任务，一个任务阻塞会影响其他任务
   - ScheduledExecutorService：jdk1.5+的定时器工具，基于多线程执行，不互相影响，复杂定时规则较弱
1. 框架级
   - spring task：不支持集群方式部署，不能做数据存储型定时任务
   - spring quartz：多线程异步执行，需要手动配置QuartzJobBean、JobDetail和Trigger等，需引入quartz包
1. 分布式定时任务
   - xxl-job
     1. 认识：开发迅速、学习简单、轻量级、易扩展、开箱即用。对quartz进行了扩展，使用mysql数据库存储数据，并且内置jetty作为RPC服务调用
        ...
        - 支持用户和权限管理
   - elastic-job
     1. 认识：采用zookeeper实现分布式协调，实现任务高可用以及分片。专门为高并发和复杂业务场景开发，apache下的项目
     1. 组成：2.x之后
        - Elastic-Job-Lite：轻量级无中心化解决方案，jar包的形式提供服务
        - Elastic-Job-Cloud
     1. 特点
        - 分布式调度协调
        - 弹性扩容缩容
        ...
        - 自诊断并修复分布式不稳定造成的问题
        - 支持并行调度
   - Saturn：唯品会开源，在Elastic-Job基础上
   - TBSchedule：阿里开发
1. k8s
   - 无需提前准备执行器，无需提前准备代码运行环境，相对于xxl-job
   - 基于容器的调度，更强的任务调度能力
### 登录鉴权
1. 认识
   - 令牌和密码
