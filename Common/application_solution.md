...
1. Open-IM
   ...
   - 实现设计
     ...
     1. 消息架构：消息模型采用经典的收件箱模型，并通过全局 seq 做消息对齐
        - msg gateway：收发消息
        ...
        - msg trasfer
        - pusher
          1. third party services：发送离线消息
        - storage
          1. redis：seq token
          1. mongodb：offline message
          1. musal：persistent message
     1. 客户端架构：Open-IMSDK，客户端SDK负责和IM服务端交互，本地数据存储和同步，消息、事件回调。开发者通过集成SDK，自行开发聊天界面UI，设置事件监听回调实现数据和UI对接
   ...
   - 应用
     1. 客户端原理
        - election如何集成im sdk？go生成exe可执行文件(或mac的可执行文件)，election直接同步启动并管理进程即可
        - ios、android如何集成im sdk？gomobile生成对应的android arr和jar包，或者xcframework文件，导入项目即可
        - web如何集成im sdk？之前是单独部署到服务器中，利用wasm本地聊天记录存储在浏览器，彻底放弃之前jssdk server服务端
          1. 借助sql.js（同样使用emscripten将SQLite编译为webassembly）和IndexedDB在Web环境中增加了消息本地存储能力，这使得SDK整体使用更加高效
#### Feed流
1. 实现方案
   ...
   - 读写混合模式：比较一下读扩散与写扩散的优缺点，不难发现两者适用场景是互补的。因此在设计后台存储时，如果能区分一下场景，在不同场景下选择最适合的方案，并且动态调整策略，就实现了读写混合模式
     1. 原理：当何炅这种粉丝量超大的人发帖时，将帖子写入何炅的发件箱，另外提取出来何炅粉丝当中比较活跃的那一批（这已经可以筛掉大部分了），将何炅的帖子写入他们的收件箱。粉丝量小的还是写扩散方式
...
1. 内容排序
   - 认识：受到状态、属性影响排序，如根据时间判断的状态流转(预告中、直播中、回放)、广告接入、特别关注、热点话题等
   - 解决方案：构建数据快照，满足当下查询
     1. “直播中”与“预告中”状态可以采用读扩散方式，“回放”状态采取写扩散方式
