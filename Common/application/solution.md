...
1. 配置方案
   - yaml
     1. 认识：YAML Ain't a Markup Language，用于跨不同语言和框架的配置文件，xml子集，01年开始，后缀.yaml或.yml
        - 缩进和冒号为主要特征，复杂
     1. 举例
        ```conf
        key: 
        ...
        ```
   - toml
     1. 认识：Tom's Obvious，Minimal Language，目标成为最小的配置文件格式
        - 语义精确，格式易于阅读
     1. 特点
        - 区分大小写
        - 文件只能包含UTF-8编码的Unicode字符
     ...
     1. 举例
        ...
        ```
   - dotenv项目：从.env文件加载配置到环境变量，认为配置要放在环境变量中，Ruby的
   - .ini
   - confd+etcd：用于做服务的配置中心
#### 数据迁移
1. 方案
