...
#### TCP
1. 认识：面向连接的有应答机制的基于字节流的可靠传输协议
   - 字节流特性：tcp会将应用层数据切割成多个数据包在网络上发送，就会失去顺序、重复包、丢包等问题，要还原就要求发送接收方都有缓冲区，凑够了再告诉上层
   - 可靠性：收包后的确认包(数据发送有回执确认)、丢包后的重发机制，只保证传输层的消息可靠性
   - 提供顺序控制、重发控制、流量控制、拥塞控制等特性
1. 组成
...
1. 建立连接
   - 认识：3次握手，目的是确认双方有正常的接收/发送能力，即都是全双工的。可靠传输更多的靠重传机制
     1. LISTEN、CLOSE是初始的状态
   - 过程
     1. 客户端：发送SYN包(seq=x)，进入SYN_SEND状态
     1. 服务端：处于LISTEN状态时收到SYN包，返回SYN(seq=y)+ACK(x+1)应答包，进入SYN_RECV状态
        - 半连接队列：服务端把处于SYN_RCVD状态的连接放入的队列，全连接就是完成握手的连接。会进行指数级数量的SYN-ACK重传，达最大次数后从半连接队列删除
     1. 客户端：收到服务端包后，发送ACK(y+1)，双方进入ESTABLISHED状态
1. 断开连接
   - 认识：4次挥手，因为连接是全双工的，需要双方都确认要关闭，双方都发了一次FIN、ACK
...
1. 传输数据
   - 超时重传
     1. 认识：每个发送的数据包都有seq号，服务端收到数据后会回复ack(seq+1)进行确认。在发送了一段时间后，如果没有收到对应的ack就认为报文丢失，进行重传
     1. tcp首部字段：序号和确认号
   ...
   - 流量控制
     1. 认识：接收方有接收窗口才能继续收数据，用于双方网速、处理能力不同的协调
        ...
        - 发送方有周期性探测包来探测大小
        - udp不需要，因为不需要确认
     1. 滑动窗口：用于流量控制，发送方根据接收方的缓冲区大小，设置自己的可发送窗口大小，处于窗口内的数据表示可发送，之外的数据不可发送
   ...
   - 粘包/拆包
     ...
     1. 相关
        - ip有分片和重组，所以没有粘包/拆包处理
        - udp没有粘包/拆包处理，要上层应用自己处理
          1. udp具体拆包工作由ip完成，接收方ip对数据包重组后传递给上层udp，所以每次udp都可以从ip层拿到一个完整的数据报文
          1. udp首部中有一个总长度字段，上层根据这个确定读取长度，tcp没有
1. 最佳实践
   - TIME_WAIT、CLOSE-WAIT巨大：并发请求太多导致，这是正常的现象，短连接太多尽量使用长连接，否则会有TCP drop风险
   - backlog调优
     1. 认识：包含半/全连接两种状态，如果满了，客户端会收到拒绝连接。计算方式最好为qps=backlog
        - 半连接：SYN queue，SYN_RCVD服务器端口状态的队列，第一次握手后
        - 全连接：Accept queue，ESTABLISHED服务器端口状态的队列，坐等程序执行accept()方法将其取走使用
     1. 半连接状态下的tcp交互步骤
        - 服务端进入SYN_RCVD状态，收到客户端的ACK后但是accept queue已满无法建立全连接，服务端丢弃后续客户端的ACK请求
   ...
   - 内核调优
     1. 缓冲区
        - 认识：分为发送、接收缓冲区，用户态send后什么时候发送由内核决定
          1. 零窗口：即接收缓冲区为0，数据包里的win改成0了，后边的再来丢包
        - 操作
          1. 查看大小
...
1. wiki
   - UDP：无连接，不可靠的基于数据报的协议，传输数据不先建立连接，不能确定是否送达，可以保证收到的大小。用于需要高速传输和实时性较高的场景
