...
1. 分类
   - LAN：局域网
     1. VLAN：虚拟局域网，分割广播域便于管理、提高性能，一个vlan里可相互通信(如arp)，不同vlan可用基于mac地址过滤的单臂路由或者三层交换机以提高安全性
...
1. wiki
   - UDP：无连接，不可靠的数据报协议，传输数据不先建立连接，不能确定是否送达，可以保证收到的大小。用于需要高速传输和实时性较高的场景，如ip电话使用tcp重发的话，就不对了，用udp的话不会有声音大幅度延迟的情况，只是产生停顿或者混乱，通常不影响，基于数据报的传输协议
   ...
   - CRC：Cyclic Redundancy Checksum，是一种纠错技术，代表循环冗余校验和，其他有奇偶校验、因特网校验
...
1. 丢包场景
   - 建立连接
     1. 半连接队列已满，新来的丢弃
   - 流量控制
     1. 流控队列长度txqueuelen不够
   - 网卡
     1. RingBuffer不够
   - 内核缓冲区
     1. 发送：阻塞调用则等，非阻塞返回EAGAIN错误让应用重试
     1. 接收：零窗口后丢包
   - 传输过程中丢包
     1. ping：查看丢包率
     1. mtr：查看每个节点丢包情况，使用ICMP包，有点节点会限制ICMP包导致不能正常显示，-u看udp包，二者结合看节点；看最后一行，才是最终正确的
### wiki
