...
1. IP
   ...
   - 分类
     1. IPv4地址范围：分为ABCDE类，特点在于是否区分、及使用不同字节数的网络地址和主机地址
        - 0.0.0.0：默认地址、任意地址、通配符地址，是无效的，表示本机上所有ipv4地址，程序用这个地址时可以对来自任意ip的请求都可响应
        - 127.0.0.1：本机地址，所以断网了可以ping通127
   ...
   - IP路由
     ...
     1. 组成
        ...
        - 主机路由：ip/32
        - 本地环回地址：以127开头的ip地址都是，127.0.0.1在ipv6下表达为::1
   - ip分片与重组
...
1. 域名
   ...
   - wiki
     ...
     1. localhost和127.0.0.1和::1
        - 本质是一个域名、一个ip，使用域名有DNS解析成本有延迟
        - localhost一般写入了本机的host文件
        - localhost在IPv6中通常解析为::1，这是IPv6下的环回地址，127.0.0.1无法利用IPv6的优势
1. DNS
...
1. QUIC
   - 认识：Quick UDP Internet Connection 快速udp互联网连接，谷歌制定的基于udp的多路并发传输协议
     1. 包含了很多和tcp相同的高级功能，深度集成了传输层安全tls协议
   - 特点
     1. 基于udp协议重新定义了连接：实现了无序、并发字节流的传输，更快的连接握手，解决了队头阻塞问题，更好的数据包丢失检测/恢复，更易于处理用户切换网络
     1. 重新定义了tls协议加密的方式：既提高了网络攻击成本，又降低了建立连接的速度
        - 对大部分内容都进行了加密，![](../../images/common/network/tcp+tls_quic_difference.webp)
          1. 以前的元数据如数据包编号、连接关闭信号等在现在quic中将仅客户端和服务器使用，传输中间件不可见
          1. 由于加密程度更高，quic的更新比tcp简单得多，即只需要更新客户端和服务器，因为中间件无论如何也无法解密元数据
        - 1个rtt就可以同时完成建链与密钥协商，以前一大堆的握手/rtt，![](../../images/common/network/http2_http3_handshake_diff.webp)
     1. 将packet、quic frame、http3 frame分离，实现了连接迁移功能，降低了高速移动设备的连接维护成本
...
1. 组成
   ...
   - 组成
     1. 报文
        - FIN：关闭连接报文
        - RST：复位报文，reset，用于释放连接，如服务端在以下情况发送RST包
          1. 客户端尝试和未对外提供服务的端口连接时返回
          1. 数据交互时程序崩溃时发送
          1. 不在其已建立的TCP连接列表内发送
          1. 超出重传后超时发送
   - 流程
...
1. 建立连接
   ...
   - 过程
     ...
     1. 服务端：处于LISTEN状态时收到SYN包，返回SYN(seq=y)+ACK(x+1)应答包，进入SYN_RECV状态
        - 半连接队列：处于SYN_RECV状态的连接的服务端队列，全连接是完成握手的连接。会进行指数级数量的SYN-ACK重传多次数后从半连接队列删除，关闭连接
        - 半连接队列满时的tcp交互
          1. 服务端进入SYN_RCVD状态，收到客户端的ACK后但是accept queue已满无法建立全连接，并丢弃后续客户端的ACK
          1. 客户端误以为连接已建立，开始调用等待至超时
          1. 服务器则等待ACK超时，会重传SYN+ACK给客户端
     1. 客户端：收到服务端包后，发送ACK(y+1)，双方进入ESTABLISHED状态
        - 服务端：将连接从半连接队列中取出，移入全连接队列
        - 进程调用accept函数，从全连接队列中取出已完成连接建立的socket连接
1. 断开连接
   ...
   - 设计原理
     ...
     1. TIME_WAIT等待2MSL的意义
        - 确保对方收到ack，保证连接正确关闭：给被动方的最后一个ACK丢失，那么被动方可重传FIN+ACK报文，主动方在2MSL内的收到的话，重传ACK，并重启2MSL计时器，保证双方能正常进入CLOSED状态
   ...
   - 重传机制
     ...
     1. 触发方式
        ...
        - 超时重传：超时计算
          ...
          1. 发送方会丢弃收到的重复确认包
   - keepalive
     1. 认识：保活机制，通过设置空闲一定时间后发送空的探测包，达到最大重试次数后断开，用于检测空闲/僵尸连接、保持nat映射(nat设备、防火墙设备)等
        - http协议的长连接和短连接，实质上是tcp协议的长连接和短连接，用的人家的嘛
        - keepalive不是tcp协议规范的一部分，但几乎所有的TCP/IP协议栈如linux、windows都实现了
        - 局限性：系统出错/异常重启/断网/断电，就没这保障了，所以对于应用程序级不靠谱
        - 应用程序级的心跳采用http也合理(ws、文件检查(写入时间戳检测是否超时)也行)，长连接方式避免了连接建立和释放带来的开销
          1. 最佳实践：不要单独实现心跳线程和单独心跳连接，因为不论是业务线程还是业务连接，挂了的话心跳还der呵的觉得正常
     1. 参数
        - KeepAlive：开关，默认关
        - tcp_keepalive_time：空闲时长，或心跳周期，默认2小时
...
1. 最佳实践
   - 网络调优
     1. 握手设置
        - 客户端：减少重发SYN包的次数，减少对服务端的重试压力，`tcp_syn_retries=5(默认)`
        - 服务端
          1. 增大backlog(这个参数由应用程序设置)、tcp_max_syn_backlog、somaxconn
          1. 开启syn cookie：在不使用syn半连接队列的情况下成功建立连接，即不分配内存，可防御SYN Flood攻击。原理为服务器根据当前状态计算一个值放在syn+ack中发出，从客户端返回的ack中取出该值验证，如果合法就认为连接建立成功
          1. 开启tcp_abort_on_overflow为1：全连接队列已满无法接纳新连接时内核的行为，0为丢弃ack(导致客户端超时并可能重试，其假定服务器暂时过载期望之后能够处理这些连接请求)，1为发送一个rst(快速失败减少客户端等待时间)
        - 开启tcp_fastopen：绕过第三次握手减少1个RTT，需双方都支持
     1. 内核连接队列设置
        - 认识：包含半/全队列，满了客户端会收到拒绝连接，计算方式最好为qps=backlog
          1. 半连接队列：SYN queue，SYN_RCVD服务器端口状态的队列，第一次握手后，`tcp_max_syn_backlog`
          1. 全连接队列：Accept queue，ESTABLISHED服务器端口状态的队列，坐等程序执行accept()方法将其取走使用
             - 参数：`somaxconn`，系统层面所有类型的监听队列(包括全连接半连接)的最大长度，更多是作为一个上限值
        - 查看
          1. 半连接队列溢出次数：`netstat -s | grep -i "SYNs to LISTEN sockets dropped"`
          1. 全连接队列溢出次数：`netstat -s | grep overflowed`
     1. 内核缓冲区设置
        - 认识：发送/接收缓冲区
          1. 发送缓冲区：tx，用户态send后要塞入网卡的发送数据需要排队 qdisc，塞入时机由内核决定，有流量控制机制
          1. 接收缓存区：rx，接收数据时会将数据暂存到RingBuffer，然后等着内核触发软中断收走；一个网卡可有多个RingBuffer
             - 零窗口：即为0，表现为数据包里的win为0，后边的再来就丢包
        - 操作
          1. 发送
             - 查看配置：`sysctl net.ipv4.tcp_wmem`，包含min、default、max
             - 查看：`ifconfig eth0`，TX下的dropped字段大于0可能丢包
             - 修改：`ifconfig eth0 txqueuelen 1500`
          1. 接收
             - 查看配置：`sysctl net.ipv4.tcp_rmem`，包含min、default、max
             - 查看：`ifconfig echo0/ethtool -S eth0|grep rx_queue_0_drops`，RX的overruns 溢出次数
             - 修改：`ethtool -G eth1 rx 4096 tx 4096`
          1. 查看是否丢包：`cat /proc/net/netstat`，TCPRcvQDrop指标，v5.9引入
   - time_wait巨大原因：短连接并发大，尽量使用长连接，否则会有tcp drop风险
     1. tcp_max_tw_buckets：超过时直接清除多余的time_wait连接的值
   - close-wait巨大
     1. 原因：不正常的连接关闭
        - 未调用或未来得及关闭连接：代码缺陷(忘了写close、执行时间长卡住了)
     1. 危害
        - 每个CLOSE_WAIT状态的连接都会占用一个文件描述符，造成新的连接无法建立/服务拒绝
        - 维护连接的内存和cpu占用增加
     1. 查看：`netstat -n | awk '/^tcp/ && /CLOSE_WAIT/ {print $5}' | cut -d: -f1 | sort | uniq -c`
   - 丢包场景及对应原理
...
1. 问题
   - 客户端发送的SYN报文为什么会被丢弃？
     1. SYN队列满了：半连接队列满了，服务端发出去的SYN + ACK报文迟迟得不到客户端的应答，久而久之就会占满服务端的半连接队列
     1. Accpet队列满了：全连接队列满了，服务端并发处理大量请求时，如果TCP Accpet队列过小，或应用程序调用accept不及时，就会造成Accpet队列已满，这时后续的连接就会被丢弃
     1. 开启tcp_tw_recycle参数即，并且处于NAT环境下
        - tcp_tw_recycle：内核中用于加速处于TCP TIME-WAIT状态的套接字回收的TCP参数，开启(=1)会采用更为积极的策略来回收这些TIME-WAIT状态的连接；搭配开启tcp_timestamps利用时间戳区分来自不同连接的旧数据包，从而安全缩短TIME-WAIT状态的持续时间，但使用了NAT网络不安全
        - tcp_tw_recycle + tcp_timestamps：会额外开启per-host的PAWS机制，可能导致SYN报文丢失
          1. PAWS：防止序列号绕回保护，tcp序列号是自增到一定程度然后归0循环使用，PAWS比较接收到的数据包时间戳与之前记录的时间戳比较，过时的就丢弃
          1. per-host的PAWS：进一步限制检查范围到仅基于源IP地址进行时间戳的校验，而不是完整的TCP四元组。即per-host的PAWS服务器会对每个源IP地址维护一个最近接收到的时间戳记录。NAT环境中多个内部主机共享一个公共出口IP地址访问互联网。由于per-host的PAWS仅基于IP地址检查时间戳，来自不同内部主机但共享相同出口IP的数据包可能会因为时间戳的不同或不连续而被错误地处理
1. wiki
   - tcp的更新和扩展
     1. tcp多路径：Multipath TCP，同时利用多条路径进行传输，提高了效率，在遇到网络拥塞或是某个链接失败时可迁移，如同时使用Wi-Fi和蜂窝网络来通信
     1. tcp fast open：为减少握手成本，用户和服务器之间交换一个特殊的cookie来验证客户端和服务器之间的连接，从而允许在握手的同时发送数据
#### UDP
...
#### 网络转换
1. 虚拟机网卡模式
   - NAT模式：虚拟机通过宿主机的网络转换与外部网络通信。虚拟网络访问外网必须通过宿主机的ip地址，不需要手动分配ip地址和子网掩码，虚拟机可ping宿主机或外部网络，反之默认不行，除非在主机上设置了端口转发
     1. 宿主机相当于有两个网卡
        - 真实网卡：链接了现实世界的真实路由器
        - 虚拟网卡：链接了一个可以认为是虚拟交换机，同时链接了虚拟机和宿主机
     1. 宿主机和虚拟交换机形成一个网段，宿主机和真实路由形成一个网段
   - 桥接模式：bridge，虚拟机如同网络中的一台独立物理机，需要手动分配ip地址和子网掩码，可以和真实主机在同一个网段，可以互相ping通
...
1. NAT
   - 认识：Network Address Translation，网络地址转换/内网穿透/公私ip转换协议，将内部的私有IP转换成公网IP，网关把自己管理的内部私网ip转为公网ip，多个内部设备对外表现为同一个公网ip，同时包括端口的映射，有映射表来记录转发路径
     1. 作用有企业对外提供内部服务、远程办公(如VPN NAT)，个人可用于个人NAS服务网络访问、安防访问
     1. 一是解决ipv4地址不够，二是安全(不在转换列表的全都拒绝)
   ...
   - 实现
     1. NAT端口映射：nat映射必须先由内部机器向外部网络发起请求才会产生
        ...
        - UPnP：Universal Plug and Play 通用即插即用，为了让各种设备能够相互无缝连接，互联网网关设备协议（IGD）。NAT穿透允许UPnP数据包在没有用户交互的情况下，无障碍的通过路由器或者防火墙
          1. 如监控直接透传到互联网，提高p2p连接成功率，远程桌面软件
        - nat-pmp：和UPnP差不多
1. VPN
   - 认识
...
1. p2p
   - 认识：peer to peer 点对点技术，不经过中间服务，双方直接通过ip和端口互发数据包，无需经过服务器转发(relay)，遵循ICE协议能打洞成功的网络都能p2p
     1. 由于防火墙、NAT等会直接忽略未向外建立连接的端口接收外部来的数据。所以需要给双方“打洞”
     1. 由于NAT设备会隐藏其背后真正通信的设备ip，所以需要”打洞“
   - 组成
     1. 创世节点
     1. 节点通信建立：bt协议、NAT穿透
   - 实现
     ...
     1. 针对NAT的解决
        ...
        - ISP的多级NAT存在：需要
1. Tailnet
   - 认识：基于WireGuard的私有覆盖网络，可以理解为“自动组网+零配置VPN+内网代理能力”的组合，类似于代理，可用于企业内控远程访问
     1. 数据路径不走中心服务器（除非 NAT 穿透失败）
#### CDN
...
1. 相关
   ...
   - EdgeOne：腾讯云的边缘安全加速服务，即cdn + waf + ddos
1. 实际应用
