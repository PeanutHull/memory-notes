...
1. prometheus
   ...
   - 组件
     ...
     1. PromQL：指标查询语言，是时序指标（metrics）查询语言
        - 原始数据：http_requests_total{method="GET", status="200"}  10234
        - promql查询示例：rate(http_requests_total[5m])
     1. LogQL：日志查询语言
        - 来源：grafana loki(对标ELK(Elasticsearch + Logstash + Kibana))的查询语言
        - 类型：日志查询 + 日志转指标
        - 数据结构：日志行 + 标签（类似prometheus）
        - 举例
          1. 日志数据：{app="api"} 2026-04-23 12:00:00 error: user login failed
          1. 查询示例：{app="api"} |= "error"
   - metric类型
...
1. 功能点
   ...
   - 扩展性：一切皆消息的消息模型
     1. 聊天消息：单聊、群聊、大群；文本、图片、视频、地理位置、自定义消息等
     1. 功能消息：已读未读、撤回、在线状态、对方正在输入、阅后即焚等
     1. 通知消息：群成员变更、添加好友、验证好友等各种通知
     1. 控制消息：连接重连、断开等
#### 技术点
1. 推送、拉取
   - 认识
...
1. 一个简单的聊天数据库设计
   - 组成：会话表、消息表
     1. 会话表
        ...
        - 未读数：通常只用redis计数
   - 机制
     1. 创建会话时，一下只插入一条会话数据，来实现双向会话数据记录；查询时查userId或者对方Id是自己的，按照时间排序
1. 如何保证消息的可靠性
1. 如何保证消息的实时性
...
1. 架构设计
   - 架构分层
     1. 多个服务，mq+http+ws+proxy+kv
   - 消息收发模型
     1. ws+http备用+http轮询+推送
     1. 消息防丢重试+ack
     1. 消息防重seq
   - 消息扩散写：采用消息扩散写的方式
     1. 每个用户都有自增seq的消息流
     1. 冷热分离的方案解决大数据量，冷数据存到廉价的SATA盘
     1. 扩散读协议设计相对复杂些
     1. im30的是群消息扩散读，已读列表维护一个表
1. 稳定性设计
   - 柔性降级：判断机器是否过载，然后后端异步重试
     1. 指数时间延迟重试
     1. 只hold住半小时的失败请求，半小时后新来的请求则直接返回前端失败
     1. 防作恶，单个用户的消耗worker数超过20%直接丢弃
   - 系统解耦：多个不同类型业务通过mq解耦，如金融版的审计模块
   - 业务隔离：不同的消息类型用不同的mq
1. 优化设计
   - 万人群
     1. 并发限制：以群id为维度限定并发上限为5。调用数被压平，没有请求无限上涨，系统稳定
     1. 合并投递：大群划分时间窗口，合并
     1. 业务降级：可以不实时更新大群是否在群的控制消息，在查询时动态拉取
   - 回执消息
     1. 合并处理
     1. 群的多个回执相互覆盖解决：使用mq串行化处理
#### Open-IM
1. IM设计
   - 一切皆消息的消息模型
   - 写扩散 + 推拉结合：解决基础问题
...
1. 数据一致性问题方案
   ...
   - CRDT 算法
     ...
     1. 核心思想：为文档中每个字符分配一个唯一的标识符，即使在删除字符时也会保留元数据
        ...
        - 使用双向链表的数据结构来实现
     1. 操作举例
        - 解决并发冲突：如从一个起点140开始，各自产生了141和142两个新的双向链表的节点，比较后142放在141的后边，不仅可描述最终的数据状态，还可以表达数据修改的顺序
...
1. 权限模型
   ...
   - RBAC
     1. 认识：Role Base Access Control，基于角色的访问控制，用户通过角色与权限进行关联，即用户-角色-权限-资源，用于相对简单的访问控制需求
     1. 组成
        - RBAC0：即基础，只包含用户、角色、权限三者的关系
        - RBAC1：引入角色继承概念，角色的权限可以设计为继承获得
        - RBAC2：引入约束规则
          1. 职责分离：SoD，防止权力集中
             - 静态职责分离：SSD，用户和角色之间的约束
               1. 互斥角色：同一个用户不能授于互斥关系的角色
               1. 基数约束：一个用户拥有的角色是有限的，一个角色拥有的许可是有限的
               1. 先决条件约束：用户想得到高级权限，必须先拥有低级权限
             - 动态职责分离：DSD，用户可以有多个角色，但同一会话不能同时启用，如登录时只能选择一个角色
          1. 基数约束
             - 一个角色最多绑定N个用户
             - 一个用户最多拥有N个角色
          1. 先决条件
...
1. 接口加密方案
   - 认识：用于各方通信时，防止数据被篡改、重放、窃听等
     1. 令牌和签名：签名的场景更多些，除了令牌的验证是否是合法的请求方以外，还可以防止请求被篡改
        - sign是sign：签名
        - token是token：鉴权、令牌
     1. 客户端泄密场景，抓包比代码破解更常见些
   - 方案
     1. 对称加解密：通常用于服务端对服务端，因为客户端不能存储密钥
     1. 非对称公私钥加解密
        - 做加密：可以客户端公钥加密，服务端私钥解密。实现密文不泄露
        - 做签名：如果做签名的话只能私钥签名，公钥验证，实际上因为非对称加密计算慢，且对加密数据长度有限制，一般用混合加密的方式，先把所有参数对称加密后用非对称加密对对称加密的内容进行加密生成数字签名，然后将原始消息和数字签名一同发出去供对方验证
     1. 一定规律混淆，增加解密成本：治标不治本，相对安全，有泄露风险，可用于客户端签名，服务端验证
        - 按照一定规则进行位运算，字符串偏移(base64后，每个字符的ASCII码进行+3操作)
1. wiki
   - 令牌和密码
...
### 框架设计
1. thinkphp的流程
   - 入口文件：public/index.php。定义目录常量，加载框架引导文件
   - 加载框架引导文件：thinkphp/start.php，引导基础文件的加载，并对应用进行初始化
   - 加载框架基础引导文件：thinkphp/base.php，该文件负责加载系统常量、引入loader类、引入环境变量、注册自动加载机制、注册异常处理机制，并加载配置文件thinkphp/convention.php
   - 运行应用：运行thinkphp/library/think/App.php下的run方法。这个方法会进行一系列的初始化操作，包括加载配置文件、加载公共文件、加载语言包、设置时区等。
   - 路由解析：根据路由规则确定要执行的控制器和方法
   - 控制器初始化：实例化控制器对象，并执行控制器的初始化方法。这个阶段通常会进行一些前置操作，如权限验证、数据初始化等
   - 方法执行：调用控制器中对应的方法来处理具体的业务逻辑。这个方法可能会读取数据库、调用模型、生成视图等。
   - 响应发送：框架将生成的HTML响应发送给客户端浏览器，完成整个请求-响应过程
1. gin的流程
   - 初始化Gin引擎：用户创建一个新的gin.Engine实例，这通常是应用程序的主路由器
   - 用户定义路由，这些路由是通过engine.Use()添加中间件和engine.GET(), engine.POST()等方法定义的。Gin使用http.ListenAndServe()或http.Server监听指定的端口，开始接受HTTP请求
   - 请求接收与解析：
     1. 当客户端发起请求时，操作系统通过TCP连接将请求传递给服务器。
     1. Gin框架使用httprouter开始解析请求的URL路径，并尝试匹配预先注册的路由规则。
   - 中间件处理：如果请求成功匹配到路由，Gin会将请求上下文和处理函数传递给中间件链条。Gin的中间件是通过闭包实现的，它们被组织成一个链条。每个中间件会调用c.Next()来传递控制权给下一个中间件，直到达到实际的处理函数。中间件可以用来进行认证、日志记录、请求验证、响应修改等操作
   - 路由处理函数执行：请求最终到达具体的路由处理函数，执行业务逻辑。处理函数完成操作后，通过c.Writer向客户端发送响应。c.Writer可以设置状态码、写入响应头和响应体。
   - 响应发送与资源清理：请求处理完成后，Gin框架会生成HTTP响应，并通过TCP连接发送给客户端。完成HTTP连接的生命周期。
1. 各个框架的扩展
   - thinkphp：事件和钩子，允许开发者在相应的时机进行扩展和定制
1. 各个框架的性能优化
   - thinkphp：配置缓存优化、路由缓存优化、数据表字段缓存、类映射加载优化
1. 各个框架的实现
   - 路由
     1. net/http：基于映射表map，比较简单
     1. gin：基于压缩前缀树
   - 协程
     1. gin：基于net/http的封装
   - 依赖注入
     1. thinkphp
        - ThinkPHP框架中的容器是一个全局的单例类，用于管理应用程序的对象实例，并提供统一的接口来访问这些对象。如在容器中注册BlogRepository类的实例，并在BlogService类的构造函数中声明需要注入BlogRepository对象，容器在实例化BlogService对象时会自动注入BlogRepository对象
        - 实现方式
          1. 构造函数注入：在类的构造函数中声明需要注入的依赖对象，容器在实例化该类时会自动注入这些依赖
          1. 方法参数注入：在类的方法中声明需要注入的依赖对象，容器在调用该方法时会自动注入这些依赖
          1. 属性注入：通过类的属性来注入依赖对象，这种方式通常需要在容器中进行额外的配置
     1. laravel
        - 通过强大的服务容器Service Container来实现，允许开发者以松散耦合的方式构建应用程序
        - 接口绑定与服务提供者：允许将接口与具体的实现类进行绑定
        - 实现方式
          1. 构造函数注入：在类的构造函数中声明需要注入的依赖对象，容器在实例化该类时会自动注入这些依赖
          1. 方法参数注入：在类的方法中声明需要注入的依赖对象，容器在调用该方法时会自动注入这些依赖
          1. 属性注入：通过类的属性来注入依赖对象，这种方式通常需要在容器中进行额外的配置
     1. gin
        - 没有显式地提供一个IoC容器类，但其内部的设计和实现已经体现了IoC的思想，Engine结构体的设计允许将不同的处理逻辑（如路由、中间件等）注入到其内部，这使得Engine可以灵活地处理各种不同的请求处理逻辑，这样就可以实现依赖注入
   - orm
     1. thinkphp：自带
