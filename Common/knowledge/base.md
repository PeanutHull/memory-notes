...
1. 一致性hash
   ...
   - 实现：一致性hash环
     ...
     1. 虚拟节点：解决服务器节点太少或者出现热点数据的hash倾斜问题。对每个服务节点计算多个哈希都放置一个虚拟节点，一个物理节点对应多个虚拟节点。虚拟节点可以在hash环上均匀分布，当一个真实节点失效退出后，它原来所承载的压力将会均匀地分散到其他节点上去，解决缓存雪崩问题
        - 分布例如：假abcd、真a、假bcda、真b、假cdab、真c、假dabc、真d，这样的排布
   - 应用
     1. es的hash路由算法
     1. haproxy的hash算法
