...
1. 理解
   ...
   - 链表和哈希表
     1. 为什么哈希表和链表经常一块使用：因为用哈希表解决链表的查询慢缺点，链表可以提供排序好的数据，如LRU缓存结构、LinkedHashMap
   - 链表和树
...
1. queue
   ...
   - 基本操作
     1. 入队：enqueue，放一个数据到尾部
     1. 出队：dequeue，从头部取一个元素
   - 实现方式
     1. 顺序队列：基于数组，一个数组中，搭配head和tail两个指针指向队列的位置，有界有限大
        - 随着不停地进行入队出队操作head和tail都会持续往后移动
        - 顺序队列的假溢出现象：前边有空位，但是后边尾指针已经挤出队列外了
          1. tail走到数组末尾时的数据搬迁逻辑：触发集中搬迁提高效率，将head到tail之间的数据平移到0到tail-head的位置
     1. 链式队列：基于链表，一个链表中，搭配head和tail两个指针指向队列的位置，无界无限大
   - 分类
     1. 双端队列
     ...
     1. 环形队列
        - 认识：ring buffer/ring queue，循环队列/环形缓冲区，固定尺寸的头尾相连的队列，是双端队列的一种实现方式
          1. 使得数据被消费的顺序性可以得到保证
          1. 头尾相连避免了队列的数据搬迁，如顺序队列的假溢出现象
          1. 必须1个线程push in，另一线程pull out，因为写入和读取的整体操作不是原子性的
        - 实现
          1. 队空队满检测：队空时head==tail，队满时tail占用一个单位，即指向的位置没有存储数据，并且着head，即`(tail + 1) % n == head`
          1. 因为缓冲区成头尾相连的环形，中间运行时写操作可能会覆盖未及时读取的数据。选择何种策略和具体场景是否允许
        - 最佳实践
          1. 适合于事先明确最大容量缓冲区的的情形：如果缓冲区的大小需要经常调整，就不适合用循环队列，因为在扩展缓冲区大小时，需要搬移其中的数据，这种场合使用链表更加合适，可用一个静态数组来充当缓冲区，无需重复申请内存
     1. 优先级队列：堆是实现的最常用方式
     1. 阻塞队列
        - 认识：在队列基础上增加了阻塞操作，做生产者-消费者模型，协调生产和消费的速度
          ...
          1. 队列已满时写数据会阻塞
     1. 并发队列
        - 认识：线程安全的队列，操作加互斥锁实现，效率较低
        - 实现
          1. 互斥锁+CAS优化：基于数组的循环队列利用CAS减少加锁的粒度
          1. Disruptor的处理方式
             - 写入数据前批量申请连续n个存储单元，线程独享这组存储单元，为后续的写入省去加锁过程，但申请时需加锁
             - 读取同写入逻辑
             - 实现上需要考虑读取超过写入范围的影响
1. skip list
...
1. 组成
   ...
   - 动态扩缩容
     1. 装载因子：表示空位比例多少，哈希表中空闲位置不多时不管哪种探测方法，冲突概率大大提高。为尽可能保证哈希表的操作效率会尽可能保证一定比例的空闲槽位
        - 装载因子过高时效率下降，过低时浪费内存，需要动态扩缩容
     1. 认识：调整装载因子
...
1. 二叉树
   ...
   - 分类
     ...
     1. 完全二叉树：满二叉树基础上，最后一层所有叶子节点都靠左排列。更满的满二叉树，因为要内存填满所以向左填满
...
1. 二叉树的应用
   - 二叉查找树
     1. 认识：二叉搜索树、二叉排序树，树中任意节点左子树中每个节点的值都要小于这个节点的值，而右子树则大于。支持动态数据集合的快速插入/删除/查找，最常用
   ...
   - 平衡二叉查找树
     1. 认识：满足平衡二叉树和二叉查找树的特点
        ...
        - 通过左右旋的方式保持左右子树的大小平衡
     1. 红黑树
        - 认识：Red-Black Tree，一种特化的平衡二叉查找树，所有语言中树结构的首选底层实现，即平衡二叉查找树的最佳。由2-3树演变来
          1. 其查询过程类似于二分查找，只与树的高度有关O(log N)
...
1. heap
   ...
   - 定义 
     ...
     1. 堆中每一个节点的值都必须大于等于（或小于等于）其子树(或左右子节点)中每个节点的值，二叉查找树和其左右子节点则是一路变大/变小
        - 二叉查找树的根节点是中位数，而堆的根节点要么是最大值要么是最小值。即堆是统一的大或小，二叉可能大可能小
   - 分类
     1. 大根堆/大顶堆：根节点最大
     1. 小根堆/小顶堆
     1. 斐波那契堆
   ...
   - 应用
     ...
     1. 利用堆求Top K
        - 针对数据集合的分类：是否有数据动态地加入到集合中
          1. 针对静态数据集合：n个数据查找前K大数据，则维护大小为K的大顶堆，顺序遍历数组与堆顶元素比较，大就插入，小不管。遍历数组O(n)、一次堆化操作O(logK)，所以最坏情况n个元素都入堆一次为O(nlogK)
...
1. 布隆过滤器
   - 认识：空间节省90%，判定时间也快，判定值存在时值可能不存在，当判定不存在时就肯定不存在
     1. 10亿数据存储只需125MB 
     1. 数据越多，误判率越高，调整哈希函数的个数、位图大小跟要存储的数据个数之间的比例，可以降的很低
     1. 布隆过滤器的数据无法删除，计数器回绕也有问题
   - 应用
     1. 不需要存储元素本身，可用于保密场合
   - 原理
     1. 设计：元素hash后映射成位阵列中的一个点。只要看这个点是不是1就可以知道集合中有没有了。需要处理hash冲突，解决方法是使用多个hash，如果有一个说不在集合中，那肯定就不在
     1. 降低哈希冲突：K个哈希函数对同一个数字进行求哈希值得到K个不同的哈希值X1、X2、X3...XK作为下标，将对应的BitMap的位都设置成true，查询时再次多次哈希求值，是否全部存在
   - 最佳实践
     1. 运维
        - 由于数据量的上升，误判率越来越高，需要支持自动扩容
        - 编程语言中的实现：java的BitSet，redis的bitMap，google的Guava工具包
     1. 场景
        - 用户的已读新闻判断，新闻特别多，读了就不会再回到未读
        - 旁路缓存中作过滤，防止缓存穿透
### 算法
1. 算法
   - 认识：解决特定问题的步骤的描述
...
1. 比较：![avatar](../../images/common/sort.jpg)
...
1. 问题
   ...
   - 如何根据年龄给100万用户排序？使用桶排序，划分140个桶，最大年龄140岁
   - 10万个大小写混合字符，要求小写在前，大写在后，大小写内部不要求
     1. 双指针遍历法：用两个指针a、b：a指针从头开始往后遍历，遇到大写字母就停下，b从后往前遍历，遇到小写字母就停下，交换a、b指针对应的元素；重复如上过程，直到a、b指针相交。对于小写字母放前面，数字放中间，大写字母放后面，可以先将数据分为小写字母和非小写字母两大类，进行如上交换后再在非小写字母区间内分为数字和大写字母做同样处理
...
1. 图的搜索
   ...
   - 深度优先算法
   - 广度优先算法
     1. 认识：BFS，上左下右一点点探索，一层层递进，每到一个点都是最短路径，往外走好多路线，往回走只有一条，可确定最短路径，深度优先走不了最短路径
        - 三种状态，已探索，未探索，待探索
        - 爬虫用到，锻炼语言理解
        - 找到确定的地方，用程序表达出来
     1. 实现方式
...
1. LRU
   - 实现
     ...
     1. 工业级实现：增加哈希表记录双向链表中位置，将增删改查都降到O(1)。![avatar](../../images/common/lru_struct.webp)
        ...
        - 插入：先看是否在链表中，在移动到尾部，不在看链表是否满，满了删除头部新的移动到尾部，没满新的移动到尾部
   - 改进
...
1. MurmurHash算法：短网址生成
   - 认识：不需考虑反向解密的难度，只需要关心计算速度和冲突概率的特性，特点是冲突概率低，2008年发明
   - 使用
...
1. BM25
   - 认识：目标是回答如果用户搜索一个词，哪篇文档最可能相关？
   - 原理：三个因素，简化公式score=TF x IDF
     1. 词频 TF：关键词在文档中出现越多，相关性越高
     1. 逆文档频率 IDF：如果一个词在很多文档里都出现，就不重要
     1. 文档长度惩罚：长文档天然会包含更多词，需要进行归一化
   - 特点
     1. 速度极快
     1. 结果稳定
     1. 无需训练
   ...
   - 最佳实践
     1. Hybrid Retrieval 混合检索：BM25 search + vector search(即semantic search) + RRF
        ```
        // 典型BM25搜索流程是
...
1. 一致性hash
   - 认识：数据哈希值范围[0, max]，k个机器划分成m个小区间（m远大于k），这样每个机器负责m/k个小区间。新机器加入时就将某几个小区间的数据搬移
     1. 可以将请求均匀分配，个数变化时依然能均匀
     1. 实现了扩缩容避免数据全部重新哈希和搬移，各个机器上数据的均衡分布，节点出错影响的数据面小
   - 实现：一致性hash环
     1. 结构
        - 顺时针查找距离这个对象hash值最近的机器，即这个对象所属的机器
        - 新机器加入时，计算哪些部分的数据需要被迁移：如abcd四个节点的bc之间加入e，那么只有b~e的数据需要搬迁
     1. 虚拟节点：解决服务器节点太少或者出现热点数据的hash倾斜问题，![avatar](../../images/common/consistence_hash_virtual_node.webp)
        - 实现：对每个服务节点计算多个的哈希值都放置一个此服务的虚节点，一个物理节点对应多个虚拟节点。数据定位算法不变，只是多了一步虚拟节点到实际节点的映射
          1. 虚拟节点在hash环上均匀分布，虚拟节点越多，hash环上缓存被均匀分布的概率就越大
          1. 当一个真实节点失效退出后，它原来所承载的压力将会均匀地分散到其他节点上去，解决缓存雪崩问题
        - 即数据不均导致数据大部分倾斜到某个节点上
        - 例如整个环的顺序分布的表现为：虚abcd、真a、虚bcda、真b、虚cdab、真c、虚dabc、真d，虚的a都映射到真a上，就一下平均了
   - 应用
     1. es的hash路由算法
     1. haproxy的hash算法
     1. redis cluster引入了哈希槽slots的概念，有2^14(16384)个slot，每个master节点负责一部分slot的管理，通过CRC-16(key)%16384决定哪个节点响应请求
     1. 基于一致性hash环的分库分表
### 程序
...
1. 目标文件
   ...
   - 库，即函数集合，存放头文件中的函数原型和对应的目标代码、链接过程的重定位信息
     1. 静态库：.a结尾，即.o文件的集合（就一.o文件的压缩包），vc下的lib文件，在程序链接阶段使用打包进程序，不能共享，使得可执行文件变大
     1. 动态库/共享库：以.so结尾，shared object，windows以dll(Dynamic Link Library 动态链接库)，macOS和ios以dylib结尾，在运行时按需加载，使得可执行文件小，同时支持了热更新，只需要存在于特定路径当中
        - ldd：列出程序所需要得动态链接库
        - c++filt：查看c++的原有值
...
1. 内存
   - 认识
     1. 函数是一段代码的封装，函数实质是这段代码的首地址，所以函数名本身就是一个内存地址
     1. 从本质来说，使用指针来访问内存和用变量访问内存是一样的，都代表一段内存地址
   - 守则内存管理
     1. 自己手动创建的，都要自己手动销毁。如某结构体里的malloc，结构体销毁但malloc出来的不会销毁，需要手动销毁
   - 常见的内存错误及其对策：内存错误处理比较麻烦，编译器不会自动发现，通常运行时才能捕捉到，大多没有明显的症状，时隐时现，增加了改错的难度
     1. 内存分配未成功就使用：内存分配会不成功。常用办法在使用内存前检查指针是否为NULL
        - 如果指针p是函数参数，在函数的入口处用assert(p!=NULL)检查
        - 如果用malloc或new申请内存，用if(p==NULL) 或if(p!=NULL)
     1. 内存分配成功但未初始化就引用。原因：一没有初始化的观念；二误以为内存的缺省初值全为零
     1. 操作内存越界
     1. 忘记释放内存，造成内存泄露
     1. 释放了内存却继续使用，有三种情况
...
#### 编程
1. 编程范式
   - 认识：编写程序的方法论、风格、看法、思维方式，规定了如何构建和组织代码
     1. 范式就像武功心法，可以更快的练成绝世神功
     1. 不同的范式的出现，目的就是为了应对不同的场景，但最终的目标都是提高生产力。就如华山派的剑宗、气宗之别，剑宗认为“剑为主，气为辅”，而气宗则反之。每个范式都会有自己的”心法”，但最终殊途同归，达到至高境界后则是剑气双修
     1. 现在是多范式融合
   - 编码组织分类
     1. 过程式
     ...
     1. 响应式：一种异步编程范式，它关注数据流和变化的传播，类似事件驱动，同时缓冲、过滤、提取、转换、合并、累积。回调地狱不好解决，和人类命令式的思考方式不同，调试排查不方便。RxJava。很方便地表达静态或动态的数据流，而相关的计算模型会自动将变化的值通过数据流进行传播，如MVVM
        - 背压：抽成真空，形成低压吸引
   - ai背景下的分类
     1. 传统：纯手动逐行编码、手动调试、依赖搜索引擎
     1. 氛围：vibe coding，ai辅助编程，基于ai agent和对话生成代码
     1. 规约：基于规范和设计共识驱动ai批量生成业务代码、多智能体协作
1. 面向对象四大特性
   - 封装
   - 继承
   - 抽象
...
1. 状态机
   - 认识：受约束的状态流转控制器，本质是把"流程逻辑"从"代码分支"抽象成"数据+规则"
     1. 防止非法状态流转
     1. 让流程可控（尤其复杂业务）
     1. 方便扩展（解耦if-else地狱）
   - 组成：状态+事件(触发条件)+转移规则(规则约束)
     1. 状态变化
     1. 触发条件（事件）
     1. 转移是否合法校验
     1. 副作用（动作）
...
1. 分类
   ...
   - 编译/解释型
     ...
     1. 混合型
        ...
        - 包含：C#
1. 组成
   - 认识：关键字、标识符、符号，程序一般由关键字、常量、变量、运算符、类型和函数组成
     1. 词法单元：关键字、标识符、字面量、运算符(+ - * /)、分隔符(; , {} ())
     1. 语法成分：常量、变量、类型、语句、函数、注释组成
     1. 标识符
        - 无意义的占位符：foo→bar→baz→qux
   - 包括
     1. 字面量：literal，是直接在代码中写出的固定值，本身就是数据，而不是一个变量或者计算表达式。“字面上”是什么就是什么。如5、"aaa"、3.14、true等
     1. 表达式：expression，表达式总是能够返回一个值
     1. 语句：statement，基本上只干活不返回，一个语句可以由n个表达式组成
     1. 注释：给人看的，会被编译器忽略
     1. 注解：给“程序”看的，就是非编程语音本体，类似于特定写法的注释，某些编程语音中有特殊的注解语法，可以被编译器或解释器识别并处理
     1. 反射：允许程序在运行时检查、修改自身的结构和行为
     1. 匿名函数：lambda，可以表示闭包，如需要一个函数但是又不想给函数命名然后再使用(即定义函数和表达式分开)，可以更简洁
        - 闭包：声明在一个函数中的函数，叫做闭包函数。能够读取其他函数内部变量的函数，同时让这些变量的值始终保持在内存中。闭包 = 函数 + 引用环境
        - lambda表达式：就是用了lambda的表达式
     1. 泛型
        - 认识：就是类型不确定，类型也是运行时的一个参数。要不得转换类型，如定义一个万能类型集合，有人捣乱存了别的类型就得做类型转换
          1. Java泛型设计原则：只要在编译时期没有出现警告，那么运行时期就不会出现ClassCastException异常
          1. 泛型编程：以一种独立于任何特定类型的方式编写代码
          1. 泛型类：把泛型定义在类上，用户使用该类的时候，才把类型明确下来
        - 特点
          1. 代码更加简洁【不用强制转换】，型是一种“代码模板”，可以用一套代码套用各种类型
          1. 程序更加健壮【只要编译时期没有警告，那么运行时期就不会出现异常】
          1. 可读性和稳定性【在编写集合的时候，就限定了类型】
1. 语言框架区别
...
1. 语言压测表现
   - 2核 4G
     1. python：使用协程有性能提升，并发极限1000，
     1. golang：性能优异，稳定性好不出错，内存占用最小，极限是python7倍
     1. java：吞吐量不如golang，计算能力差不多，内存占用很大
     1. nodejs：吞吐量不如java，计算能力强
   - php
...
1. 认识：受到乔姆斯基文法的启发，从汇编语言发展到用编译来实现高级编程语言
   - JIT：just in time，即时编译、热重载，表示运行时将指令转为二进制机器码，jit可以将opcode直接转为机器码，大幅提升性能
   - AOT：Ahead-Of-Time，本地编译
1. 编译器
   - GCC：一统之前纷争的天下
...
1. 编译模式
   - 静态编译：生成二进制机器码文件，直接加载运行。Go/C/C++/Rust
   - 动态编译：生成二进制机器码文件，运行时需要查找并链接系统的.dll/.so库文件。C/C++的常见方式
   - 字节码编译：生成平台无关的字节码，由虚拟机解释/JIT编译执行，Java/C#
   - 不编译(解释型)：由解释器逐行读取并执行，Python/JS/Ruby
1. 反编译
   - 工具
     1. IDA：还原出汇编指令
     1. F5插件：还原出高级语言
...
1. wiki
   ...
   - 服务端游戏开发和互联网开发
     1. 游戏：注重同步实时计算、低延迟的要求高(如40人弹道计算，有直接拿u3d做服务端的)；发展慢、不互通、代码重用性差
     1. 互联网：基建好、互相学习、进步快；有难度如音视频实时通信(大数据量传输、丢包)、机房数据同步、容灾、AI增强、im通信(tg的非常多的大文件上传分享、数据同步)
     1. 结论：不要用自己复杂的东西比较别人浅层的东西
### 数据
#### 类型
1. 字节序
   - 认识：存储方式，`0x2211使用两个字节储存：高位字节是0x22，低位字节是0x11`
     1. 电路先处理低位字节，效率比较高。除了计算机内部处理，其他场合都是大端字节序，如网络传输和文件存储
     1. 原则
        - 只有读取的时候，才必须区分字节序，其他情况都不用考虑
   - 分类
     1. 大端字节序：高位字节在前，低位字节在后，这是人类读写数值的方法
     1. 小端字节序：低位在前，高位在后，即以0x1122形式储存
1. 文件格式
   - 二进制：如 PNG
   - 数据：如 SQLite
   - 文档：如 Markdown
     1. MDX：markdown的扩展，支持jsx(JavaScript XML)语法，可以在md文件中直接写react组件，mdx文件可被编译成jsx文件，用于展示markdown内容和react组件
        - 用于：经常需要文档 + demo + 组件展示，一份文件就能解决
   - 配置：如toml
   - 编程语言：如 JavaScript
#### 二进制
1. 进制转换
...
### 思想
1. 软件构建方式的革命：用更少代码解决更大问题，用更少运维创造更多价值
   - 优秀的库扩展能力边界，明智的开发者决定边界在哪
### wiki
1. 时间
