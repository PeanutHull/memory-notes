...
1. 思想
   ...
   - 软件退化的根源：不要怪罪于版本迭代和需求变更，如果每次软件变更时，适时地进行解耦，进行功能扩展，再实现新的功能，就能保持高质量的软件设计。但如果在每次软件变更时没有调整程序结构，而是在原有的程序结构上不断地塞代码，软件就会退化
   - 掌握平衡：一方面要满足当前需求，另一方面要刚刚满足需求，从而使设计最简化、代码最少。这样做，不仅软件设计质量提高了，设计难点也得到了大幅度降低
1. 认识
   - 认识：DDD，领域驱动设计，将过程式业务流程设计变为面向对象式。解决软件更新迭代变坏的理念
...
1. 落地
   ...
   - 概念
     ...
     1. 仓储：repository，连接领域层和基础设施层的桥梁，一般将仓储接口定义放在领域层，仓储的具体实现放在基础设施层
1. wiki
   - 真正决定软件复杂性的是设计方法
   - 康威定律：微服务的拆分要对应组织
   - 需求分析阶段：四色原型分析模式
