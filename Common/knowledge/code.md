...
1. 单例模式
   ...
   - 克隆和继承
        ...
        ```
1. 工厂模式
   - 理解：Factory，定义一个用于创建对象的接口，让子类决定实例化哪一个类，使实例化延迟到其子类来决定
...
1. 原型模式
   ...
   - 代码
        ...
        ```
1. 适配器模式
   - 理解：Adapter，把一个“接口不兼容”的现有对象，转换成你想要的接口，是“事后兼容”
     1. 屏蔽多来源细节，统一调用
   ...
   - 示例
    ...
    ```
1. 桥接模式
   - 理解：Bridge，把“抽象”和“实现”拆成两个独立维度，通过组合连接起来，让它们可以分别变化，是“事前解耦”
     1. 就是解耦了排列组合
   ...
   - 示例
    ...
    ```
1. 组合模式
   - 理解：Composite，单个和整体统一对待，专治树形结构。把“单个对象”和“一组对象”统一成同一种接口，让调用方不用区分叶子节点还是组合节点
     1. 关键点是文件和目录都实现同一个接口
   ...
   - 示例
    ...
    ```
1. 装饰器模式
   - 理解：Decorator，不修改原对象代码，通过“套一层壳”的方式，动态给对象增加功能
...
1. 外观模式
   ...
   - 示例
    ...
    ```
1. 享元模式
   - 理解：Flyweight，把大量重复对象中“相同的部分”抽出来共享，避免重复创建对象，主要目的是省内存
   ...
   - 示例
    ...
    ```
1. 代理模式
   - 理解：Proxy，不让调用方直接访问真实对象，而是先经过一个“代理对象”，由代理决定要不要、什么时候、怎么访问真实对象
   ...
   - code
    ...
    ```
1. 注册树模式
   - 理解：Registry，注册表模式，为了解决全局共享，将创建好的对象注册到注册树上，需要访问直接从注册树上查询即可
   ...
   - 示例
    ...
    ```
1. 责任链模式
   - 理解：Chain of Responsibility，把一堆处理器串成链，请求沿着链往下传，每个处理器只负责自己那一小段逻辑。即把请求交给责任链的下一个节点
     1. 可以灵活设置链条
     1. 每一步可以随时终止流程
   ...
   - demo
    ...
    ```
1. 命令模式
   - 理解：Command，把“一次操作”封装成一个对象，让操作可以被存储、排队、重试、记录、撤销，而不是立即执行
   ...
   - 示例
    ...
    ```
1. 解释器模式
   - 理解：Interpreter，把一套“规则/表达式”解析成程序可以执行的逻辑。是自定义的一套小语法/DSL，让不同的“表达式对象”解释它
   ...
   - 示例
    ...
    ```
1. 迭代器模式
   - 理解：Iterator，把“怎么遍历一堆数据”这件事，从业务代码里抽出去。即调用方只管不断Next()，不用知道数据底层是数组、链表、分页接口还是数据库游标
   ...
   - 示例
    ...
    ```
1. 中介者模式
   - 理解：Mediator，多个业务模块不要互相直接调用，统一通过一个“中间人”协调
   ...
   - 示例
    ...
    ```
1. 备忘录模式
   - 理解：Memento，在不暴露对象内部实现的情况下，保存对象某一时刻的状态，并且以后可以恢复，给业务对象做一个“快照/撤销点”
   ...
   - 示例
    ...
    ```
1. 观察者模式
   - 理解：Observer，一个对象发生变化后，自动通知一批依赖它的对象
   ...
   - 示例
     ...
     1. 精简写法
        ...
        ```
1. 状态模式
   - 理解：State，同一个对象，因为当前状态不同，对同一个操作表现出不同的行为。把`if status == xxx`这种状态判断，拆到不同的状态对象里，因为状态变多判断就变多了
   ...
   - 示例
    ...
    ```
1. 策略模式
   - 理解：Strategy，把一组“可以互相替换的业务算法”抽出来，调用方根据场景选择其中一个
   ...
   - 示例
    ...
    ```
1. 模板方法模式
   - 理解：Template Method，把一套固定流程放在“模板”里，流程中的某些步骤留给具体实现去决定
   ...
   - 示例
    ...
    ```
1. 访问者模式
   - 理解：Visitor，对象类型比较稳定，但针对这些对象的“操作”经常增加时，把操作从对象里抽出去
