...
1. wiki
   ...
   - 混沌工程：系统太过于复杂时，无法测试每一个分支，并且成千上万个组件组合在一起后总会产生意想不到的”化学反应“，与其等着系统出问题，不如我们主动制造一些问题，看看系统会怎么反应
     1. 理解
        - 通过应用一些经验探索的原则主动发现问题，来观察弹性系统是如何反应的
     1. 方法：如模拟整个IDC当掉、选择一部分网络连连接注入特定时间的延迟、随机让一些函数抛出异常、榨干cpu
     1. 开发框架
        - ChaosBlade：阿里开发，一个核心Go CLI接收指令，利用众多插件即各领域java、c等执行器执行相应的故障操作，如jvm控制、k8s控制、网络、io等
          1. chaosblade-exec-jvm：Java Agent + 字节码增强
          1. chaosblade-exec-cplus：GDB调试技术
          1. chaosblade-exec-docker：Docker API
### 高并发
1. 如何设计
   - 系统进行拆分
   - 业务设计上进行分离
