...
1. 分类
   - 互联网：Internet，多个网络连接形成更大的网络，即网际网。由ARPANET(为可靠军用通信诞生)发展而来
   - LAN：局域网
...
### 网络协议
#### 基础
1. ARP：Address Resolution Protocol，地址解析协议，通过ip拿mac地址，先局域网广播询问物理地址，之后缓存一段时间，ipV6中用NDP代替
   - RARP：反向地址转换协议
   - ARP响应：发现是自己的ip，告诉别人
   - ARP欺骗：由于建立在互相信任基础上，攻击者可伪造目标机器的ARP信息，导致数据会通过攻击者转发
1. IP
   - 分类：ABCDE类地址，特点在于是否区分、以及使用了不同字节数的网络地址和主机地址
     1. 0.0.0.0：默认地址，所有地址
     1. 255.255.255.255：限制广播地址，指所有主机，不能被路由器所转发
     1. 127.0.0.1 本机地址
     1. 224.0.0.1 组播地址，224.0.0.1特指所有主机，224.0.0.2特指所有路由器，IRDP(Internet路由发现协议），使用组播功能功能，那么你的主机路由表中应该有这样一条路由。
     1. 169.254.x.x：DHCP服务器发生故障的默认分配地址
     1. 10.x.x.x、172.16.0.0---172.31.255.254、192.168.x.x：私有地址，免得和共有冲突
   - ip划分
   ...
   - IP路由：是设备根据 IP 地址对数据进行转发的操作。当一个数据包到达路由器时，路由器根据数据包的目的地址查询路由表，根据查询结果将数据包转发出去，这个过程就是
     ...
     1. 默认路由：为 0.0.0.0/0 或 default，也是无效的，也是表示本机上所有IPV4地址
     1. 主机路由：ip/32 被称为主机路由
     1. 回环地址：以127开头的ip地址都是，127.0.0.1在ipv6下表达为::1
   - ip分片与重组
1. ICMP
   - 认识：Internet Control Message Protocol，Internet控制报文协议，ping通过ICMP消息
...
1. 传输数据
     1. 超时重传：客户端每次发送数据包都有seq号，服务端收到数据后会回复ack(seq+1)进行确认。在发送了一段时间后，如果没有收到对应的ack回复就认为报文丢失，会重传
        - TCP首部字段：序号和确认号
     1. 连续ARQ：连续发送数据包，然后逐一确认
        - 停止等待协议：你一下我一下，这样效率太低
     1. 快速重传：服务端主动告诉客户端重传
     1. 滑动窗口：用于流量控制，服务端告诉发送端自己还有多少缓冲区可以接收数据，发送端就根据服务端的处理能力来发送数据，加速数据传输
        - TCP首部字段：窗口大小，表示接收方的剩余缓冲区大小
        - 步骤
          1. 发送方需要根据接收方的缓冲区大小，设置自己的可发送窗口大小，处于窗口内的数据表示可发送，之外的数据不可发送
          1. 当窗口内的数据接收到确认回复时，整个窗口会往前移动，直到发送完成所有的数据
     1. 拥塞控制：基于整个网络来考虑，因为重传可能带来网络风暴，拥塞策略算法主要包括：慢启动，拥塞避免，拥塞发生，快速恢复
     1. 累积确认：累积一定量数据包后一次性回复，
     1. 选择确认
     1. keepalive：保活机制
...
1. 其他
   ...
   - socks：防火墙安全会话转换协议，只提供两端连接和数据包传递，在握手阶段通知客户端，用于客户端与外网的中间传递(如防火墙)，5支持udp，4是tcp
   - socks5：代理协议，是会话层的协议，最新协议是版本5，增加支持UDP、验证、IPv6，1990年开发，此后作为RFC的开放标准
