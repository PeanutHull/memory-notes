...
1. 集线器
   - 认识，中继器，物理层延长网络的设备，数据广播发送，功能是对信号进行放大
     1. 同一时刻网络上只能传输一组数据帧的通讯，如果发生碰撞还得重试，很不高效
1. 网桥
   - 认识：数据链路层延长网络的设备
1. 交换机
   - 认识：数据链路层的负责根据mac地址进行数据帧转发的设备
...
1. http2.0
   ...
   - 多路复用
     ...
     1. 双方可以同时互相发送数据
   - grpc：google主导开发的基于http2.0的cs型的rpc框架，可以实现多路复用，就是并发的请求和接收，比webSocket高效
     1. 简单模式：单调的顺序请求、响应
     1. 双向数据流模式：请求、响应并行起来
   - SPDY
