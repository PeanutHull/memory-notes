...
1. wiki
   - 1M宽带指1Mbps，即兆比特每秒，除以8就是KB每秒，MB/S是Mbps的8倍，再扣12%的信息头标识等各种控制讯号，上限为112KB/s
...
1. 均衡策略
   - 静态负载均衡算法
     1. 随机、加权随机
     1. 轮询、加权轮询、平滑的加权轮询算法：能力高的后端服务权重更高
     1. 哈希与一致性哈希
        - 哈希：同样请求ip、同样请求url对应相同后端服务
   - 动态负载均衡算法
     1. 最少响应时间
     1. 最少连接数
     1. 最少活跃请求数
...
1. 分片和副本
   - 分片集群的缺点：配置复杂：相比副本集，配置和管理分片集群相对复杂，需要根据业务需求进行节点数量和规格的选择，不适用于多字段随机组合查询场景
副本集的缺点：容量和性能限制：副本集的容量受到单节点的限制，当数据量增长到一定程度时，可能会出现容量和性能瓶颈
   - 分片集群具有横向扩展能力，支持实例级别的备份和回档，保证数据高可靠。而副本集则通过部署多个服务器存储数据副本来达到高可用的能力，但容量和写流量的瓶颈限制。
     1. 如果需要横向扩展、高性能和高可用性，且对配置和管理有一定的能力，可以选择分片集群。
     1. 如果业务不需要大规模横向扩展，副本集更为适合。如果业务需要多字段随机组合查询，并且不容易选择合适的片键索引，建议使用副本集。
### 服务治理
#### 接口
1. 指标
   - qps：调用量
