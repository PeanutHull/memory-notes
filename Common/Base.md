...
1. hash table
   - 认识：哈希表/散列表/map/映射/字典/键值对。通过key和value直接访问的数据结构
     ...
     1. 有哈希表碰撞攻击
     1. 拉链法，数组链表，数组的每个成员包括一个指针，指向一个链表的头。根据元素的一些特征把元素分配到不同的链表中去，也是根据这些特征，找到正确的链表，再从链表中找出这个元素
     1. 位图
   - 分类
     1. set：集合，不重复的元素
...
1. trie
   ...
   - 组成
     1. 结构：根节点不包含任何信息，每个节点表示一个字符串中的字符，从根节点到红色节点的一条路径表示一个字符串
     1. 存储
        - 经典：类似哈希表，通过一个下标与字符一一映射的数组，来存储子节点的指针，没有子节点就存NULL，实现字符的串联
          1. 查找：
   - 操作
     1. 构造：一个个字符串分解后插入到树中
     1. 查找
