...
1. 二叉树的应用
   - 二叉查找树
     1. 认识：二叉排序树，树中任意节点左子树中每个节点的值都要小于这个节点的值，而右子树则大于。支持动态数据集合的快速插入/删除/查找，最常用
        - 复杂度最差退化为链表为O(n)，复杂度和树的高度成正比，类似平衡二叉查找树的高度接近logn，复杂度也比较稳定是O(logn)
     1. 操作
     ...
     1. 支持重复数据存储：用对象中某个字段作为键值构建二叉查找树，就会存在重复的数据
        ...
        - 重复数据放右子树
          ...
          1. 删除时按照以上查找依次删除
   - 平衡二叉树
     1. 认识：需要构建一种不管怎么删除/插入，在任何时候都能保持任意节点左右子树都比较平衡的二叉查找树。AVL树、红黑树，通过左右旋的方式保持左右子树的大小平衡

   - 红黑树：所有语言中树结构的首选底层实现，2-3树和红黑树的等价性，数据量小性能优势比链表不大
     1. 快速增删改查
     1. 可实现快速的插入、删除、查找
   - 哈夫曼树
   - Trie
   - union find：并查集
1. heap
   - 认识：堆，一般叫二叉堆，类似二叉树，子节点值不大于或不小于父节点值，是一种完全二叉树，最常用数组存储
