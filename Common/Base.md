...
1. queue
   ...
   - 环形队列
     1. 认识：ring buffer/ring queue，环形缓冲区/循环队列，是固定尺寸的头尾相连的标准先入先出的数据结构，可用于2个线程之间传递数据，必须遵循1个线程push in，另一线程pull out的原则
        - 背景：解决多线程共享数据使用互斥锁mutex作同步的开销
        - 是Deque的一种实现方式
     1. 特点
   ...
   - 双端队列
     1. 认识：Deque Double-ended queue，也称首尾链表，元素可以在队列两端添加或删除的线性表
