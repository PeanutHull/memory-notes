...
1. 时间复杂度
   ...
   - O(nlogn)
     ...
     1. 二叉排序树
1. 冒泡
   - 认识：两两相邻的数进行比较，如果反序就交换，然后每次进行n-1数量的比较
     1. 空间平均O(1)最差(元素全逆序)O(n)，即在交换元素时那个临时变量所占的内存空间
1. 插入
1. 选择
   - 认识：每次从待排序的数据中选出最大/小的元素，放在起始位置，直到排完
     1. 空间O(1)
1. 希尔
   - 认识：把待排序数据根据增量分成几个子序列，对子序列插入排序直到增量为1，直接插入排序。增量的排序一般是数组长度的一半，再变为原来增量的一版，直到为1
     1. 时间最坏O(n^2)/平均O(nlog2n)，空间O(1)

1. 归并
   - 认识：将两个或以上有序表合并为新的有序表，即把待排序序列分为若干有序的子序列，再把有序的子序列合并为整体有序序列
     1. 时间最坏/平均O(nlog2n)，空间O(n)
     1. 将数据分为几部分，分别单独排序，再把所有数据归并。归并就是找到相同的记下来，作为结果，一个个的挑。同时也实现了去重，只看数据头非常省空间
1. 快速
   - 认识：通过一趟排序将数据分割为两部分，一部分比另一部分所有数据都小，递归对两部分快排
     1. 时间最坏O(n^2)/平均O(nlog2n)，空间最坏O(n)/平均O(log2n)
1. 堆
