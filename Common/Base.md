...
1. 使用方式分类
     ...
     1. 堆
        ...
        - 二项堆
1. 顺序线性表
   - 认识：array/数组/向量，最简单最广泛，使用连续内存来存储，所有元素类型相同或衍生，可下标访问，下标从0开始
   - 优点
     1. 按照索引查询元素速度快
     1. 按照索引遍历数组方便
   - 缺点
...
1. set：集合，不重复的元素
1. hash table
   - 认识：哈希表/散列表/map/映射/字典/键值对。通过关键码和值(key和value)直接进行访问的数据结构。散列表就是把Key通过一个固定的算法函数既所谓的哈希函数转换成一个整型数字，然后就将该数字对数组长度进行取余，取余结果就当作数组的下标，将value存储在以该数字为下标的数组空间里，这种存储空间可以充分利用数组的查找优势来查找元素，所以查找的速度很快。因为哈希表是基于数组衍生的数据结构，在添加删除元素方面是比较慢的，所以很多时候需要用到一种数组链表来做，也就是拉链法。拉链法是数组结合链表的一种结构，较早前的hashMap底层的存储就是采用这种结构，直到jdk1.8之后才换成了数组加红黑树的结构
     1. 需要考虑其他问题：哈希冲突
1. graph
...
1. 组成
   ...
   - 评估方式
     1. 指标：多方面的因素
        ...
        - 数据特点
     1. 复杂度分析
        - 时间复杂度：执行算法需要的计算量，是问题规模n的函数f(n)，记做T(n)=O(f(n))，即计算次数，最好/最坏/平均/均摊。如常数阶`O(1)`、线性阶`O(n)`、平方阶`O(n^2)`、对数阶`O(log2n)`
          1. 得出算法的计算次数公式
...
1. 应用算法
   ...
   - LFU：最不经常最少使用算法，需要额外空间记录字块的使用频率
