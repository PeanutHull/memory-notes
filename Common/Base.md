...
1. heap
   ...
   - 实现
     ...
     1. 插入：插入后要堆化，有两种从下往上和从上往下，即不断递归对比节点和其子节点的大小然后交换，直到符合，交换可以避免移动出现树节点的空洞
        - 特点：把新插入的数据放到数组的最后，然后从下往上堆化，操作时间复杂度O(logn)
        - 代码
            ```java
            public class Heap {
     ...
     1. 删除：删除节点后，开始堆化，即交换，和插入相同
        - 特点：把数组中的最后一个元素放到堆顶，然后从上往下堆化，操作时间复杂度O(logn)
        - 代码
            ```java
...
1. 堆排序
   - 认识：原地的排序算法，堆排序比快速排序的时间复杂度还要稳定，但实际使用不如其快
     ...
     1. 不稳定，互换操作会改变顺序
   - 步骤
     1. 建堆：原地建堆，将下标从二分之一到1的节点依次进行从上到下的堆化操作，因为对于完全二叉树后边部分的数据都是叶子节点，下一个操作排序自然就会处理，只处理倒数第二层就可以形成完全二叉树
        - 方式一：从前往后依次插入堆中
        - 方式二：从后往前处理，从上往下堆化。
          1. 叶子节点不需要堆化
          1. 堆化过程比较和交换的节点个数跟节点高度h成正比，这样建堆的复杂度就是每个节点的高度的求和公式
        - 方式二代码
            ```java
            ...
            ```
     1. 排序：建堆后是大顶堆，堆顶跟最后一个元素交换，然后进行堆化，排序就完成了
        ```java
...
1. 为什么快速排序要比堆排序性能好
   ...
   - 同样数据排序过程中，堆排序交换次数多
     1. 建堆后数据反而更无序
1. 应用
   - topK：如求数据频率最高的k个数，建立k个数的最小顶堆，然后剩余数据和堆顶比较，如果比堆顶大则替换堆顶并重新调整最小顶堆
   - 流里边的中值、中位数
##### wiki
1. 问题
