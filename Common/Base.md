...
1. queue
   - 认识：队列，只能在一端添加元素，而在另一端取出元素，先进先出
   - 双端队列
     1. 认识：Deque Double-ended queue，也称首尾链表，元素可以在队列两端添加或删除的线性表，那就不是FIFO了，是一种具有队列和栈的性质的数据结构
     ...
     1. 实现
        - 顺序存储结构：基于数组
        - 链式存储结构：基于线性表的单链表，只能从后端进从前端出
   - 环形队列/环形缓冲区
     1. 认识：ring buffer/ring queue，是固定尺寸的头尾相连的标准先入先出的数据结构，可用于2个线程之间传递数据，必须遵循1个线程push in，另一线程pull out的原则
        - 背景：解决由于多线程共享数据使用互斥锁mutex作同步有一定的开销
        - 是Deque的一种实现方式
     1. 特点
        - 一个数据元素被读取出后其余数据元素不需移动其存储位置
...
1. 分类
   - 常见排序算法
     1. 双重循环：用第一个数与后面的所有数进行比较，然后把最小的数放到第一个位置，然后用第二个数与后面的所有数进行比较，然后把第二个最小的数放到第二个位置，然后用第三个数....最后用倒数第二个数进行比较，这样最终形成从小到大的排列顺序，效率最低，时间复杂度O(n*m)
     1. 冒泡排序：两两相邻的数进行比较，如果反序就交换，然后每次进行n-1数量的比较。时间最坏/平均O(n^2)，空间O(1)
