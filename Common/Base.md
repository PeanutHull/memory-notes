...
1. list
   ...
   - linked-list
     ...
     1. 缺点
        - 因为含有大量的指针域，占用空间较大
        - 查找元素需要遍历链表来查找，非常耗时
     1. 适用场景：数据量较小，需要频繁增加，删除操作的场景
   - cyclic-linked-list
     1. 认识：循环链表，特殊单链表，尾节点指向了头节点
     1. 优点：从链尾到链头比较方便，如约瑟夫问题相比用单链表解决更简洁
   - doubly-linked-list
     1. 认识：双向链表，包括一个关键字key、两个指针next和prev，两个方向
     1. 特点：占用更多空间，但是某些条件新增删除更高效
        - 删除时需要修改待修改节点的前驱节点，更高效
        - 有序链表下记录上次查询节点，平均只需要查找一半的数据即可得出
   - doubly-cyclic-linked-list
     1. 认识：你懂！![avatar](../images/common/doubly-cyclic-linked-list.webp)
   - 缓存淘汰策略
     1. FIFO
...
1. queue
   - 认识：队列，只能在一端添加元素，而在另一端取出元素，FIFO 先进先出
   - 环形队列/环形缓冲区
...
1. 算法
   ...
   - 评估方式
     1. 指标：多方面的因素
        ...
        - 数据特点
     1. 复杂度分析
        - 时间复杂度：执行算法需要的计算量，是问题规模n的函数f(n)，记做T(n)=O(f(n))，即计算次数，最好/最坏/平均/均摊
          1. 指标
             - 线性阶`O(n)`
             - 常数阶`O(1)`
             - 平方阶`O(n^2)`
