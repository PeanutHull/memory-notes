...
1. 组成
   ...
   - lambda：就是一个匿名函数，可以表示闭包，如需要一个函数但是又不想给函数命名然后再使用(即定义函数和表达式分开)，可以更简洁
     1. 闭包：声明在一个函数中的函数，叫做闭包函数。能够读取其他函数内部变量的函数，同时让这些变量的值始终保持在内存中
...
1. 编程范式
   ...
   - 分类
     ...
     1. 函数式
        ...
        - 组成：mapping（映射），reducing（归纳），piplining（管道），recursing（递归），currying（科里化）
        - 特点：避免副作用，不会依赖也不会改变当前函数以外的
          1. 简介快速，隔离方便管理
          1. 较占用运行资源，性能较差
     1. 面向对象
...
1. protobuf
   - 认识：语言无关、平台无关、可扩展的序列化结构数据的方法，二进制格式，用于网络通信、通用数据交换、数据存储等场景，google推出
     1. 比xml、json小几倍，快几十倍
     1. 更高的时间效率和空间效率，适合对数据大小和传输速率比较敏感的场合使用
   - 使用
     1. 创建.proto文件，定义数据结构
     1. protoc编译.proto 文件生成读写接口
     1. 调用接口实现序列化、反序列化以及读写
   - 比较
     1. XML、JSON、protobuf 都具有数据结构化和数据序列化的能力
     1. XML、JSON更注重数据结构化，关注人类可读性和语义表达。protoBuf更注重数据序列化，关注效率、空间、速度，人类可读性差，语义表达能力不足（为保证极致的效率，会舍弃一部分元信息）
     1. protoBuf的应用场景更为明确，XML、JSON的应用场景更为丰富
1. msgpack
   - 认识：高效的二进制序列化格式，比json更快、更小
     1. msgpack可以直接序列化类对象，protobuf需要先写描述映射文件(.proto)
### wiki
1. 端口号
   - DNS：53
   - FTP：20、21
