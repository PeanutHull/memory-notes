...
1. 理解
   ...
   - 工程中但凡用到动态插入、删除、查找数据的场景都可以用红黑树。不过实现起来比较复杂，自己写代码实现难度会有些高，这个时候用跳表来替代
#### 线性表
1. array
   ...
   - 缓存淘汰策略
     ...
     1. LRU：Least Recently Used，最近最少使用算法
        ...
        - 工业级实现：结合hash table和list来记录每个数据的位置，将增删改查的时间复杂度都降到O(1)。![avatar](../images/common/lru_struct.webp)
          1. 每个节点会在两条链中，双向链表和散列表中的拉链，前驱和后继指针是为了将节点串在双向链表中(会跨不同槽位的链表哦，指明在链表中的位置)，hnext指针是为了将节点串在散列表的拉链中(指明是哪个槽位的链表)，即改造hash_table的链表结构，数据data只存一份，但是指针各有用处
          1. 查找：O(1)，然后移动到链表尾
...
1. queue
   ...
   - 双端队列
     1. 认识：Deque Double-ended queue，也称首尾链表，元素可以在队列两端入队或出对的线性表
     1. 基本操作
        - 头：读、入、出
        - 尾：读、入、出
   - 环形队列
     1. 认识：ring buffer/ring queue，循环队列/环形缓冲区，固定尺寸的头尾相连的队列，是Deque的一种实现方式
        - 一个数据元素被读出、写入后其余数据元素不需移动其存储位置
        - 队空队满检测：队空时head==tail，队满时tail占用一个单位挨着head
        - 因为缓冲区成头尾相连的环形，写操作可能会覆盖未及时读取的数据。选择何种策略和具体场景是否允许
        - 背景：解决多线程共享数据使用互斥锁mutex作同步的开销，可用于2个线程之间传递数据，必须遵循1个线程push in，另一线程pull out的原则
        - 适合于事先明确最大容量缓冲区的的情形：如果缓冲区的大小需要经常调整，就不适合用环形缓冲区，因为在扩展缓冲区大小时，需要搬移其中的数据，这种场合使用链表更加合适，可用一个静态数组来充当缓冲区，无需重复申请内存
   - 阻塞队列
     1. 认识：在队列基础上增加了阻塞操作，类似生产者-消费者模型
        - 队列为空时取数据会阻塞
        - 队列已满时写数据会阻塞
   - 并发队列
     1. 认识：线程安全的队列，可以在操作上加锁，但是循环队列利用CAS原子操作更高效
1. skip list
   - 认识：跳表，就是给有序链表建立多层级的索引，实现快速定位查找。可实现快速的插入、删除、查找，甚至可以替代红黑树，redis的有序集合SortedSet利用其+散列表实现
...
1. hash table
   - 认识：哈希表/散列表/map/映射/字典/键值对。通过key和value直接访问的数据结构
     1. 底层存储基于数组，利用数组支持按照下标随机访问数据的特性，发挥快速的查找优势，插入/删除慢，是数组的一种扩展，时间复杂度O(1)，用到了数组链表，即拉链法
     1. 把key通过固定的哈希函数转换成整型数字，然后将该数字对数组长度进行取余，取余结果当作数组下标，将value存储在以该数字为下标的数组空间里，称为桶bucket/槽slot
     1. 有散列表碰撞攻击
     1. 位图
   - 分类
     1. set：集合，不重复的元素
   - 散列函数：决定了散列冲突的概率，也就决定散列表的性能
     1. 基本要求
        - 计算结果是非负整数，因为要用在数组下边
     ...
     1. 优化要求
        ...
        - 太复杂会太耗时间
     1. 设计方法：如每个字母的ASCll码值“进位”相加，然后再跟散列表大小求余、取模，作为散列值。还有直接寻址法、平方取中法、折叠法、随机数法
   - 冲突解决
     1. 比较
   ...
   - 装载因子：表示空位比例多少，散列表中空闲位置不多时不管哪种探测方法，冲突概率大大提高。为尽可能保证散列表的操作效率会尽可能保证一定比例的空闲槽位
...
1. 最佳实践
   - 工业级哈希表的要求
     ...
     1. 性能稳定，极端情况下，散列表的性能也不会退化到无法接受的情况
   - LinkedHashMap：通过双向链表和散列表实现，Linked指双向链表。按照访问时间排序的LinkedHashMap本身就是一个支持LRU缓存淘汰策略的缓存系统，二者实现原理也一模一样
...
1. 二叉树
   ...
   - 分类
     ...
     1. 完全二叉树：叶子节点都在最底下两层，最后一层的叶子节点都靠左排列，并且除了最后一层，其他层的节点个数都最大，更满的满二叉树，因为要内存填满所以向左填满
   - 实现
...
1. 二叉树的应用
   - 二叉查找树
     ...
     1. 支持重复数据存储：用对象中某个字段作为键值构建二叉查找树，就会存在重复的数据
        - 每个节点通过链表和支持动态扩容的数组等数据结构，把值相同的数据都存储在同一个节点上
        - 重复数据放右子树
          1. 查找时遇到值相同的节点不停止继续在右子树中查找，直到叶子节点停止
          1. 删除时按照以上查找依次删除
   - 平衡二叉查找树
     1. 认识：二叉树中任意一个节点的左右子树的高度相差不能大于1
...
1. heap
   ...
   - 实现
     ...
     1. 插入：插入后要堆化，有两种从下往上和从上往下，即不断对比然后交换，直到符合
...
1. 其他
   ...
   - 递归树：借助递归树来分析递归算法的时间复杂度
#### 图
1. graph
   - 认识：图，由节点的有穷集合V和边的集合E组成，在图结构中常常将节点称为顶点，边是顶点的有序偶对，若两个顶点之间存在一条边，就表示这两个顶点具有相邻关系
   - 组成
     1. 存储
        - 邻接矩阵
        - 邻接表
     ...
     1. 时间复杂度
        ...
        - 量级：递增
          ...
          1. 非多项式量级：n越大执行时间急剧增加
             ...
             - 阶乘：`O(n!)`
     1. 空间复杂度
        - 认识：类似上边，表示算法的存储空间与数据规模之间的增长关系，包含代码/输入数据/辅助变量，就是看需要的空间和n的关系就可以了，要简单很多
          1. 注意这里的存储空间是指除了原本的数据存储空间外，算法运行还需要额外的存储空间
1. 组成
   - 思想
     1. 基本算法思想
        - 贪心
        - 分治：Divide Conquer
        - 动态规划：Dynamic Programming DP，将一个问题拆成几个子问题，分别求解这些子问题，即可推断出大问题的解。（严格定义：如果给定某一阶段的状态，则在这一阶段以后过程的发展不受这阶段以前各段状态的影响。）
   ...
   - 其他
     ...
     1. 秦九韶算法
1. 应用算法
   - 哈希算法：也称散列算法，是一个单向函数，可以把任意长度的输入数据转化为固定长度的输出
...
1. 时间复杂度
   - O(n^2)：平方阶
     1. 冒泡：两两相邻的数进行比较，如果反序就交换，然后每次进行n-1数量的比较。时间最坏/平均O(n^2)，空间O(1)
     1. 插入
     1. 选择：每次从待排序的数据中选出最大/小的元素，放在起始位置，直到排完。时间最坏/平均O(n^2)，空间O(1)
     1. 希尔：把待排序的根据增量分成几个子序列，对子序列插入排序，直到增量为1，直接插入排序。增量的排序一般是数组长度的一半，再变为原来增量的一版，直到为1。时间最坏O(n^2)/平均O(nlog2n)，空间O(1)
   - O(nlogn)
     1. 归并：将两个或以上有序表合并为新的有序表，即把待排序序列分为若干有序的子序列，再把有序的子序列合并为整体有序序列。时间最坏/平均O(nlog2n)，空间O(n)
        - 将数据分为几部分，分别单独排序，再把所有数据归并。归并就是找到相同的记下来，作为结果，一个个的挑。同时也实现了去重，只看数据头非常省空间
     1. 快速：通过一趟排序将数据分割为两部分，一部分比另一部分所有数据都小，递归对两部分快排。时间最坏O(n^2)/平均O(nlog2n)，空间最坏O(n)/平均O(log2n)
     1. 堆：把待排序的元素按照大小在二叉树位置上排列，排序规则为父节点大于等于子节点，即堆化过程，如果根节点最大叫大根堆，最小小根堆，把根节点拿出来再堆化循环到最后一个节点。时间最坏/平均O(nlog2n)，空间O(1)
     1. 二叉排序数
   - O(n)
     1. 计数
     1. 基数
...
1. 深度优先算法
1. 广度优先算法
   - 认识：BFS，上左下右一点点探索，一层层递进，每到一个点都是最短路径，深度优先走不了最短路径
